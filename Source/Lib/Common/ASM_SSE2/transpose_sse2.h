...
static INLINE void transpose_32bit_8x8_sse2(const __m128i *const in, __m128i *const out) {
    const __m128i a00 = _mm_unpacklo_epi32(in[0], in[2]);
    const __m128i a01 = _mm_unpacklo_epi32(in[1], in[3]);
    const __m128i a10 = _mm_unpacklo_epi32(in[4], in[6]);
    const __m128i a11 = _mm_unpacklo_epi32(in[5], in[7]);
    const __m128i a20 = _mm_unpacklo_epi32(in[8], in[10]);
    const __m128i a21 = _mm_unpacklo_epi32(in[9], in[11]);
    const __m128i a30 = _mm_unpacklo_epi32(in[12], in[14]);
    const __m128i a31 = _mm_unpacklo_epi32(in[13], in[15]);

    const __m128i a40 = _mm_unpackhi_epi32(in[0], in[2]);
    const __m128i a41 = _mm_unpackhi_epi32(in[1], in[3]);
    const __m128i a50 = _mm_unpackhi_epi32(in[4], in[6]);
    const __m128i a51 = _mm_unpackhi_epi32(in[5], in[7]);
    const __m128i a60 = _mm_unpackhi_epi32(in[8], in[10]);
    const __m128i a61 = _mm_unpackhi_epi32(in[9], in[11]);
