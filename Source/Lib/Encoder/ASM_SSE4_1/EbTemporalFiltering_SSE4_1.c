...
static AOM_FORCE_INLINE void xx_load_and_pad(uint16_t *src, int col, int block_width, __m128i output[2]) {
        ...
        if (context_ptr->tf_ctrls.use_fixed_point && (ss_x_shift == 1) && (ss_y_shift == 1)) {
            for (int i = 0; i < block_height; i++) {
                ...
                for (int j = 0; j < block_width; j += 4) {
                    int     xx   = (j << 1);
                    __m128i line1 = _mm_cvtepu16_epi32(_mm_loadl_epi64((__m128i *)(luma_sq_error + yy * SSE_STRIDE + xx)));
                    __m128i line2 = _mm_cvtepu16_epi32(_mm_loadl_epi64((__m128i *)(luma_sq_error + yy * SSE_STRIDE + xx + 4)));
                    __m128i sum = _mm_hadd_epi32(line1, line2);

                    line1 = _mm_cvtepu16_epi32(_mm_loadl_epi64((__m128i *)(luma_sq_error + (yy + 1) * SSE_STRIDE + xx)));
                    line2 = _mm_cvtepu16_epi32(_mm_loadl_epi64((__m128i *)(luma_sq_error + (yy + 1) * SSE_STRIDE + xx + 4)));
    ...
    for (int i = 0; i < block_height; i++) {
        ...
        for (int j = 0; j < block_width; j += 4) {
            ...
            if (context_ptr->tf_ctrls.use_fixed_point) {
                ...
                __m128 combined_error_ps = _mm_castsi128_ps(
                                                _mm_unpacklo_epi64(
                                                    _mm_castps_si128(combined_error_ps1),
                                                    _mm_castps_si128(combined_error_ps2)));
    ...
    if (plane != PLANE_TYPE_Y) {
        ...
        if (context_ptr->tf_ctrls.use_fixed_point && (ss_x_shift == 1) && (ss_y_shift == 1)) {
            for (int i = 0; i < block_height; i++) {
                ...
                for (int j = 0; j < block_width; j += 4) {
                    int     xx   = (j << 1);
                    __m128i line1 = _mm_loadu_si128((__m128i *)(luma_sq_error + yy * SSE_STRIDE + xx));
                    __m128i line2 = _mm_loadu_si128((__m128i *)(luma_sq_error + yy * SSE_STRIDE + xx + 4));
                    __m128i sum = _mm_hadd_epi32(line1, line2);

                    line1 = _mm_loadu_si128((__m128i *)(luma_sq_error + (yy + 1) * SSE_STRIDE + xx));
                    line2 = _mm_loadu_si128((__m128i *)(luma_sq_error + (yy + 1) * SSE_STRIDE + xx + 4));
                    sum   = _mm_add_epi32(sum, _mm_hadd_epi32(line1, line2));
