...
#endif
#if OPT_RPS_CONSTR_2
        const uint16_t num_ref_from_cur_mg = get_num_refs_in_one_mg(scs_ptr->static_config.hierarchical_levels, scs_ptr->mrp_ctrls.referencing_scheme) + 1; //+1: to accomodate one for a delayed-I
#else
...
#if OPT_LD_M13
        input_data.rtc_tune = (enc_handle_ptr->scs_instance_array[instance_index]->scs_ptr->static_config.pred_structure == SVT_AV1_PRED_LOW_DELAY_B) ? true : false;
#endif
...
#if FIX_LAYER_SIGNAL
        scs_ptr->tf_params_per_type[0].max_num_future_pics = MIN((1 << scs_ptr->static_config.hierarchical_levels), svt_aom_tf_max_ref_per_struct(scs_ptr->static_config.hierarchical_levels, 0, 1));
#else
...
#if FIX_LAYER_SIGNAL
        scs_ptr->tf_params_per_type[1].max_num_past_pics = MIN((1 << scs_ptr->static_config.hierarchical_levels), svt_aom_tf_max_ref_per_struct(scs_ptr->static_config.hierarchical_levels, 1, 0));
        scs_ptr->tf_params_per_type[1].max_num_future_pics = MIN((1 << scs_ptr->static_config.hierarchical_levels), svt_aom_tf_max_ref_per_struct(scs_ptr->static_config.hierarchical_levels, 1, 1));
#else
...
#if FIX_LAYER_SIGNAL
        scs_ptr->tf_params_per_type[2].max_num_past_pics = MIN((1 << scs_ptr->static_config.hierarchical_levels) / 2, svt_aom_tf_max_ref_per_struct(scs_ptr->static_config.hierarchical_levels, 2, 0));
        scs_ptr->tf_params_per_type[2].max_num_future_pics = MIN((1 << scs_ptr->static_config.hierarchical_levels) / 2, svt_aom_tf_max_ref_per_struct(scs_ptr->static_config.hierarchical_levels, 2, 1));
#else
...
void derive_tf_params(SequenceControlSet *scs_ptr) {
    ...
    if (scs_ptr->static_config.pred_structure == SVT_AV1_PRED_LOW_DELAY_P || scs_ptr->static_config.pred_structure == SVT_AV1_PRED_LOW_DELAY_B) {
        if (do_tf == 0)
            ...
            tf_level = enc_mode <= ENC_M10 ? 1 : enc_mode <= ENC_M12 && (scs_ptr->input_resolution >= INPUT_SIZE_720p_RANGE) ? 2 : 0;

        tf_ld_controls(scs_ptr, tf_level);
        return;
    }
#endif
    if (do_tf == 0) {
...
void set_param_based_on_input(SequenceControlSet *scs_ptr)
{
   ...
   // Open loop intra done with TPL, data is not stored
    ...
    if (scs_ptr->static_config.rate_control_mode == SVT_AV1_RC_MODE_VBR || scs_ptr->static_config.rate_control_mode == SVT_AV1_RC_MODE_CBR ||
        scs_ptr->static_config.pred_structure == SVT_AV1_PRED_LOW_DELAY_B || scs_ptr->static_config.pass != ENC_SINGLE_PASS || scs_ptr->static_config.enc_mode >= ENC_M10)
        scs_ptr->enable_dg = 0;
    else
        scs_ptr->enable_dg = scs_ptr->static_config.enable_dg;
...
#if OPT_LD_MRP2
    else if (scs_ptr->static_config.pred_structure == SVT_AV1_PRED_LOW_DELAY_B && scs_ptr->static_config.enc_mode <= ENC_M12) {
        mrp_level = 6;
    }
    else if (scs_ptr->static_config.enc_mode <= ENC_M7 || scs_ptr->static_config.pred_structure == SVT_AV1_PRED_LOW_DELAY_B) {
    ...
    scs_ptr->low_latency_kf = ((scs_ptr->static_config.pred_structure == SVT_AV1_PRED_LOW_DELAY_P
         || scs_ptr->static_config.pred_structure == SVT_AV1_PRED_LOW_DELAY_B) &&
        scs_ptr->static_config.enc_mode <= ENC_M10)
        ? 1
        : 0;
#endif
}
...
void copy_api_from_app(
    ...
    derive_input_resolution(
        ...
        scs_ptr->max_input_luma_width * scs_ptr->max_input_luma_height);
#if OPT_LD_M13
    if (scs_ptr->static_config.pred_structure == SVT_AV1_PRED_RANDOM_ACCESS && scs_ptr->static_config.enc_mode > ENC_M12 && input_resolution <= INPUT_SIZE_360p_RANGE) {
        scs_ptr->static_config.enc_mode = ENC_M12;
    ...
    // Dynamic GoP
    scs_ptr->static_config.enable_dg = ((EbSvtAv1EncConfiguration*)config_struct)->enable_dg;
    ...
    if (scs_ptr->static_config.pass == ENC_SINGLE_PASS && scs_ptr->static_config.pred_structure == SVT_AV1_PRED_LOW_DELAY_B) {
#if OPT_LD
        if (scs_ptr->static_config.hierarchical_levels != 2) {
            scs_ptr->static_config.hierarchical_levels = 2;
