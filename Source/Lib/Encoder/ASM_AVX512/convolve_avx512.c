...
static INLINE void sr_x_round_store_32x2_avx512(const __m512i res[2],
    uint8_t *const dst,
...
static INLINE void sr_x_round_store_64_avx512(const __m512i res[2],
    uint8_t *const dst) {
    __m512i r[2];
...
static INLINE void sr_y_round_store_64_avx512(const __m512i res[2],
    uint8_t *const dst) {
    __m512i r[2];
...
static INLINE void sr_y_round_store_64x2_avx512(const __m512i res[4],
    uint8_t *const dst,
    const int32_t dst_stride) {
...
static INLINE void sr_y_round_store_32x2_avx512(const __m512i res[2],
    uint8_t *const dst,
    const int32_t dst_stride) {
    __m512i r[2];
...
static INLINE void sr_y_2tap_32x2_avx512(const uint8_t *const src,
    const int32_t src_stride,
    const __m512i coeffs[1],
    __m256i *const s, uint8_t *const dst,
    const int32_t dst_stride) {
    __m512i r[2];
...
static INLINE void sr_y_2tap_64_avx512(const uint8_t *const src,
    const __m512i coeffs[1],
...
static INLINE void sr_y_2tap_64_avg_avx512(const uint8_t *const src,
    const __m512i s0, __m512i *const s1,
    uint8_t *const dst) {
...
void eb_av1_convolve_y_sr_avx512(const uint8_t *src, int32_t src_stride,
    ...
    if (is_convolve_2tap(filter_params_y->filter_ptr)) {
        ...
        if (subpel_y_q4 != 8) {
            if (w <= 8) {
                ...
                if (w == 32) {
                    ...
                    do {
                        sr_y_2tap_32x2_avx512(src_ptr + src_stride,
                            src_stride,
                            coeffs_512,
                            &s_256,
                            dst,
                    ...
                    do {
                        sr_y_2tap_64_avx512(src_ptr + src_stride,
                            coeffs_512,
                            s_512[0],
                            &s_512[1],
                            dst);
                    ...
                    s_512[0][0] =
                        _mm512_loadu_si512((__m512i *)(src_ptr + 0 * 64));
                    s_512[0][1] =
                        _mm512_loadu_si512((__m512i *)(src_ptr + 1 * 64));
            ...
            if (w <= 8) {
                ...
                do {
                    sr_y_2tap_64_avg_avx512(
                        src_ptr + src_stride, s_512[0], &s_512[1], dst);
                    sr_y_2tap_64_avg_avx512(src_ptr + 2 * src_stride,
                        s_512[1],
                        &s_512[0],
                        dst + dst_stride);
                    src_ptr += 2 * src_stride;
                ...
                do {
                    sr_y_2tap_64_avg_avx512(src_ptr + src_stride + 0 * 6,
                        s_512[0][0],
                        &s_512[1][0],
                        dst + 0 * 6);
                    sr_y_2tap_64_avg_avx512(src_ptr + src_stride + 1 * 64,
                        s_512[0][1],
                        &s_512[1][1],
                        dst + 1 * 64);
        ...
        if (w <= 4) {
            ...
            if (w == 8) {
                ...
                prepare_half_coeffs_4tap_avx512(
                    filter_params_y, subpel_y_q4, coeffs_512);
        ...
        if (w <= 4) {
            ...
            if (w == 32) {
                ...
                s_256[2] =
                    _mm256_loadu_si256((__m256i *)(src_ptr + 2 * src_stride));
                s_256[3] =
                    _mm256_loadu_si256((__m256i *)(src_ptr + 3 * src_stride));
                s_256[4] =
                    _mm256_loadu_si256((__m256i *)(src_ptr + 4 * src_stride));
                ...
                do {
                    ...
                    s_512[0] =
                        _mm512_loadu_si512((__m512i *)(s + 0 * src_stride));
                    s_512[1] =
                        _mm512_loadu_si512((__m512i *)(s + 1 * src_stride));
                    s_512[2] =
                        _mm512_loadu_si512((__m512i *)(s + 2 * src_stride));
                    s_512[3] =
                        _mm512_loadu_si512((__m512i *)(s + 3 * src_stride));
                    s_512[4] =
                        _mm512_loadu_si512((__m512i *)(s + 4 * src_stride));
        ...
        if (w <= 4) {
            ...
            if (w == 32) {
                ...
                s_256[4] =
                    _mm256_loadu_si256((__m256i *)(src_ptr + 4 * src_stride));
                s_256[5] =
                    _mm256_loadu_si256((__m256i *)(src_ptr + 5 * src_stride));
                s_256[6] =
                    _mm256_loadu_si256((__m256i *)(src_ptr + 6 * src_stride));
                ...
                do {
                    ...
                    s_512[4] =
                        _mm512_loadu_si512((__m512i *)(s + 4 * src_stride));
                    s_512[5] =
                        _mm512_loadu_si512((__m512i *)(s + 5 * src_stride));
                    s_512[6] =
                        _mm512_loadu_si512((__m512i *)(s + 6 * src_stride));
...
static INLINE void sr_x_2tap_32x2_avx512(const uint8_t *const src,
    const int32_t src_stride,
    const __m512i coeffs[1],
    uint8_t *const dst,
    const int32_t dst_stride) {
...
static INLINE void sr_x_2tap_64_avx512(const uint8_t *const src,
    const __m512i coeffs[1],
    uint8_t *const dst) {
    __m512i r[2];
...
static INLINE void sr_x_2tap_64_avg_avx512(const uint8_t *const src,
    uint8_t *const dst) {
...
static INLINE void sr_x_6tap_32x2_avx512(
    const uint8_t *const src, const int32_t src_stride, const __m512i coeffs[3],
...
static INLINE void sr_x_6tap_64_avx512(const uint8_t *const src,
    const __m512i coeffs[3],
...
SIMD_INLINE void sr_x_8tap_32x2_avx512(
    const uint8_t *const src, const int32_t src_stride, const __m512i coeffs[4],
...
SIMD_INLINE void sr_x_8tap_64_avx512(const uint8_t *const src,
    const __m512i coeffs[4],
...
void eb_av1_convolve_x_sr_avx512(const uint8_t *src, int32_t src_stride,
    uint8_t *dst, int32_t dst_stride, int32_t w,
    int32_t h, InterpFilterParams *filter_params_x,
    InterpFilterParams *filter_params_y,
    const int32_t subpel_x_q4,
    ...
    if (is_convolve_2tap(filter_params_x->filter_ptr)) {
        ...
        if (subpel_x_q4 != 8) {
            if (w <= 8) {
                ...
                if (w == 32) {
                    do {
                        // Slower than avx2.
                        sr_x_2tap_32x2_avx512(
                            src_ptr, src_stride, coeffs_512, dst, dst_stride);
                        src_ptr += 2 * src_stride;
                        dst += 2 * dst_stride;
                        y -= 2;
                    } while (y);
                    ...
                    do {
                        sr_x_2tap_64_avx512(
                            src_ptr + 0 * 64, coeffs_512, dst + 0 * 64);
                        sr_x_2tap_64_avx512(
                            src_ptr + 1 * 64, coeffs_512, dst + 1 * 64);
                        src_ptr += src_stride;
                        dst += dst_stride;
                    } while (--y);
                }
        ...
        if (is_convolve_6tap(filter_params_x->filter_ptr)) {
            ...
            if (w <= 16) {
                ...
                filt_256[0] =
                    _mm256_load_si256((__m256i const *)filt1_global_avx);
                filt_256[1] =
                    _mm256_load_si256((__m256i const *)filt2_global_avx);
                filt_256[2] =
                    _mm256_load_si256((__m256i const *)filt3_global_avx);
                ...
                if (w == 32) {
                    ...
                    do {
                        sr_x_6tap_64_avx512(src_ptr, coeffs_512, filt_512, dst);
                        sr_x_6tap_64_avx512(
                            src_ptr + 64, coeffs_512, filt_512, dst + 64);
                        src_ptr += src_stride;
                        dst += dst_stride;
                    } while (--y);
                }
            ...
            if (w <= 16) {
                ...
                filt_256[3] =
                    _mm256_load_si256((__m256i const *)filt4_global_avx);
                filt_256[0] =
                    _mm256_load_si256((__m256i const *)filt1_global_avx);
                filt_256[1] =
                    _mm256_load_si256((__m256i const *)filt2_global_avx);
