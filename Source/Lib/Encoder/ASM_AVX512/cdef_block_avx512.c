...
static INLINE void cdef_filter_block_8x8_16_pri_avx512(
    const uint16_t *const in, const __m128i damping, const int32_t po,
    const __m512i row, const __m512i strength, const __m512i pri_taps,
    ...
    *sum = _mm512_add_epi16(
        *sum, _mm512_mullo_epi16(pri_taps, _mm512_add_epi16(q0, q1)));
}

static INLINE void cdef_filter_block_8x8_16_sec_avx512(
    const uint16_t *const in, const __m128i damping, const int32_t so1,
    const int32_t so2, const __m512i row, const __m512i strength,
    const __m512i sec_taps, __m512i *const max, __m512i *const min,
    __m512i *const sum) {
    const __m512i mask = _mm512_set1_epi16(0x3FFF);
    ...
    *sum = _mm512_add_epi16(
        *sum,
        _mm512_mullo_epi16(sec_taps,
            _mm512_add_epi16(_mm512_add_epi16(q0, q1),
                _mm512_add_epi16(q2, q3))));
}

void eb_cdef_filter_block_8x8_16_avx512(
...

    {
        ...
        // Primary near taps
        cdef_filter_block_8x8_16_pri_avx512(in,
            pri_d,
            po1,
            row,
            pri_strength_256,
        ...
        // Secondary near taps
        cdef_filter_block_8x8_16_sec_avx512(in,
            sec_d,
            s1o1,
            s2o1,
            row,
        ...
        _mm_storeu_si128((__m128i *)&dst[0 * dstride],
            _mm512_castsi512_si128(res));
        _mm_storeu_si128((__m128i *)&dst[1 * dstride],
            _mm512_extracti32x4_epi32(res, 1));
        _mm_storeu_si128((__m128i *)&dst[2 * dstride],
            _mm512_extracti32x4_epi32(res, 2));
        _mm_storeu_si128((__m128i *)&dst[3 * dstride],
            _mm512_extracti32x4_epi32(res, 3));
    }

    {
        const __m512i row =
            loadu_u16_8x4_avx512(in + 4 * CDEF_BSTRIDE, CDEF_BSTRIDE);
        __m512i sum, res, max, min;
        ...
        // Primary near taps
        cdef_filter_block_8x8_16_pri_avx512(in + 4 * CDEF_BSTRIDE,
            pri_d,
            po1,
            row,
            pri_strength_256,
        ...
        _mm_storeu_si128((__m128i *)&dst[4 * dstride],
            _mm512_castsi512_si128(res));
        _mm_storeu_si128((__m128i *)&dst[5 * dstride],
            _mm512_extracti32x4_epi32(res, 1));
        _mm_storeu_si128((__m128i *)&dst[6 * dstride],
            _mm512_extracti32x4_epi32(res, 2));
        _mm_storeu_si128((__m128i *)&dst[7 * dstride],
            _mm512_extracti32x4_epi32(res, 3));
    }
}
