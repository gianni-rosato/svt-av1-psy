...
typedef void (*EbAv1EncodeLoopFuncPtr)(PictureControlSet *pcs, EncDecContext *ed_ctx,
                                       ...
                                       uint32_t *cnt_nz_coeff, uint32_t component_mask,
                                       uint16_t *eob);
...
static void encode_pass_update_intra_mode_neighbor_arrays(
    ...
    if (component_mask & PICTURE_BUFFER_DESC_CHROMA_MASK) {
        // Intra Luma Mode Update
        svt_aom_neighbor_array_unit_mode_write(intra_chroma_mode_neighbor_array,
                                               &chroma_mode,
                                               ((org_x >> 3) << 3) / 2,
                                               ((org_y >> 3) << 3) / 2,
                                               width_uv,
...
static void encode_pass_update_inter_mode_neighbor_arrays(NeighborArrayUnit *mode_type_na,
    ...
    // Skip Flag
    svt_aom_neighbor_array_unit_mode_write(skipNeighborArray,
                                           skip_flag,
                                           org_x,
                                           org_y,
                                           bwidth,
...
static void encode_pass_update_recon_sample_neighbour_arrays(
    ...
    uint32_t org_x, uint32_t org_y, uint32_t width, uint32_t height, uint32_t bwidth_uv,
    uint32_t bheight_uv, uint32_t component_mask, Bool is_16bit) {
    ...
    if (is_16bit == TRUE) {
        ...
        if (component_mask & PICTURE_BUFFER_DESC_CHROMA_MASK) {
            // Recon Samples - Cb
            svt_aom_neighbor_array_unit16bit_sample_write(
...
static void av1_encode_loop(PictureControlSet *pcs, EncDecContext *ed_ctx, SuperBlock *sb_ptr,
    ...
    const uint32_t input_cb_offset = (((round_origin_y + input_samples->org_y) >> 1) *
                                      input_samples->stride_cb) +
        ((round_origin_x + input_samples->org_x) >> 1);
    const uint32_t input_cr_offset = (((round_origin_y + input_samples->org_y) >> 1) *
    ...
    const uint32_t pred_cb_offset = (((pred_samples->org_y + round_origin_y) >> 1) *
                                     pred_samples->stride_cb) +
        ((pred_samples->org_x + round_origin_x) >> 1);
    const uint32_t pred_cr_offset = (((pred_samples->org_y + round_origin_y) >> 1) *
    ...
    const uint32_t scratch_cb_offset =
        ROUND_UV(ed_ctx->blk_geom->tx_org_x[is_inter][blk_ptr->tx_depth][ed_ctx->txb_itr]) / 2 +
        ROUND_UV(ed_ctx->blk_geom->tx_org_y[is_inter][blk_ptr->tx_depth][ed_ctx->txb_itr]) / 2 *
            SB_STRIDE_UV;
    ...
    if (component_mask == PICTURE_BUFFER_DESC_FULL_MASK ||
        ...
        if (ed_ctx->md_skip_blk) {
            cnt_nz_coeff[0] = 0;
            eob[0]          = 0;
            ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
                .quantized_dc[0][ed_ctx->txb_itr] = 0;
        } else {
            svt_residual_kernel8bit(
                ...
                ed_ctx->blk_geom->tx_width[blk_ptr->tx_depth][ed_ctx->txb_itr],
            ...
            svt_aom_estimate_transform(((int16_t *)residual16bit->buffer_y) + scratch_luma_offset,
                                       ...
                                       NOT_USED_VALUE,
                                       ed_ctx->blk_geom->txsize[blk_ptr->tx_depth][ed_ctx->txb_itr],
            ...
            int32_t seg_qp = pcs->ppcs->frm_hdr.segmentation_params.segmentation_enabled
                ? pcs->ppcs->frm_hdr.segmentation_params
                      .feature_data[ed_ctx->blk_ptr->segment_id][SEG_LVL_ALT_Q]
                : 0;

            ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
                .quantized_dc[0][ed_ctx->txb_itr] = svt_aom_quantize_inv_quantize(
                sb_ptr->pcs,
                ed_ctx->md_ctx,
                ((TranLow *)transform16bit->buffer_y) + coeff1d_offset,
                NOT_USED_VALUE,
        ...
        ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
            .y_has_coeff[ed_ctx->txb_itr] = cnt_nz_coeff[0] ? TRUE : FALSE;
    ...
    if (component_mask == PICTURE_BUFFER_DESC_FULL_MASK ||
        ...
        if (blk_ptr->prediction_mode_flag == INTRA_MODE &&
            ...
            uint32_t             recon_luma_offset = (recon_samples->org_y + round_origin_y) *
            ...
            svt_cfl_luma_subsampling_420_lbd(
                ...
                ed_ctx->blk_geom->bwidth_uv == ed_ctx->blk_geom->bwidth
                    ? (ed_ctx->blk_geom->bwidth_uv << 1)
                    : ed_ctx->blk_geom->bwidth,
                ed_ctx->blk_geom->bheight_uv == ed_ctx->blk_geom->bheight
                    ? (ed_ctx->blk_geom->bheight_uv << 1)
                    : ed_ctx->blk_geom->bheight);
            int32_t round_offset =
                ((ed_ctx->blk_geom->tx_width_uv[blk_ptr->tx_depth][ed_ctx->txb_itr]) *
                 (ed_ctx->blk_geom->tx_height_uv[blk_ptr->tx_depth][ed_ctx->txb_itr])) /
                2;
            ...
            svt_subtract_average(
                ...
                ed_ctx->blk_geom->tx_height_uv[blk_ptr->tx_depth][ed_ctx->txb_itr],
                round_offset,
                svt_log2f(ed_ctx->blk_geom->tx_width_uv[blk_ptr->tx_depth][ed_ctx->txb_itr]) +
                    svt_log2f(ed_ctx->blk_geom->tx_height_uv[blk_ptr->tx_depth][ed_ctx->txb_itr]));
        ...
        if (ed_ctx->md_skip_blk) {
            ...
            eob[1]          = 0;
            ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
                .quantized_dc[1][ed_ctx->txb_itr] = 0;
            cnt_nz_coeff[2]                       = 0;
            eob[2]                                = 0;
            ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
                .quantized_dc[2][ed_ctx->txb_itr] = 0;
        } else {
            int32_t seg_qp = pcs->ppcs->frm_hdr.segmentation_params.segmentation_enabled
                ? pcs->ppcs->frm_hdr.segmentation_params
                      .feature_data[ed_ctx->blk_ptr->segment_id][SEG_LVL_ALT_Q]
            ...
            svt_aom_estimate_transform(
                ...
                NOT_USED_VALUE,
                ed_ctx->blk_geom->txsize_uv[blk_ptr->tx_depth][ed_ctx->txb_itr],
                &ed_ctx->three_quad_energy,
                EB_EIGHT_BIT,
                ...
                DEFAULT_SHAPE);
            ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
                .quantized_dc[1][ed_ctx->txb_itr] = svt_aom_quantize_inv_quantize(
                sb_ptr->pcs,
                ed_ctx->md_ctx,
                ((TranLow *)transform16bit->buffer_cb) + ed_ctx->coded_area_sb_uv,
                NOT_USED_VALUE,
            ...
            svt_aom_estimate_transform(
                ...
                DEFAULT_SHAPE);
            ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
                .quantized_dc[2][ed_ctx->txb_itr] = svt_aom_quantize_inv_quantize(
                sb_ptr->pcs,
                ed_ctx->md_ctx,
                ((TranLow *)transform16bit->buffer_cr) + ed_ctx->coded_area_sb_uv,
                NOT_USED_VALUE,
        ...
        ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
            .u_has_coeff[ed_ctx->txb_itr] = cnt_nz_coeff[1] ? TRUE : FALSE;
        ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
            .v_has_coeff[ed_ctx->txb_itr] = cnt_nz_coeff[2] ? TRUE : FALSE;
...
*
**********************************************************/
static void av1_encode_loop_16bit(PictureControlSet *pcs, EncDecContext *ed_ctx, SuperBlock *sb_ptr,
                                  uint32_t org_x, uint32_t org_y,
                                  EbPictureBufferDesc *pred_samples, // no basis/offset
                                  EbPictureBufferDesc *coeff_samples_sb, // sb based
                                  EbPictureBufferDesc *residual16bit, // no basis/offset
...

{
    ...
    const uint32_t input_cr_offset =
        ...
        ROUND_UV(ed_ctx->blk_geom->tx_org_y[is_inter][blk_ptr->tx_depth][ed_ctx->txb_itr]) / 2 *
            SB_STRIDE_UV;
    const uint32_t pred_luma_offset = ((pred_samples16bit->org_y + org_y) *
...
static void av1_encode_generate_recon(EncDecContext *ed_ctx, uint32_t org_x, uint32_t org_y,
    ...
    if (component_mask & PICTURE_BUFFER_DESC_LUMA_MASK) {
        {
            uint32_t pred_luma_offset = (pred_samples->org_y + org_y) * pred_samples->stride_y +
                (pred_samples->org_x + org_x);
            if (ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
                        .y_has_coeff[ed_ctx->txb_itr] == TRUE &&
                blk_ptr->skip_mode == FALSE) {
                uint8_t *pred_buffer = pred_samples->buffer_y + pred_luma_offset;
                svt_aom_inv_transform_recon8bit(
                    ((int32_t *)residual16bit->buffer_y) + ed_ctx->coded_area_sb,
                    pred_buffer,
                    pred_samples->stride_y,
                    pred_buffer,
                    pred_samples->stride_y,
    ...
    if (component_mask & PICTURE_BUFFER_DESC_CHROMA_MASK) {
        ...
        uint32_t round_origin_x     = (org_x >> 3) << 3; // for Chroma blocks with size of 4
        uint32_t round_origin_y     = (org_y >> 3) << 3; // for Chroma blocks with size of 4
        uint32_t pred_chroma_offset = (((pred_samples->org_y + round_origin_y) >> 1) *
                                       pred_samples->stride_cb) +
            ((pred_samples->org_x + round_origin_x) >> 1);
        ...
        //**********************************
        if (ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
                    .u_has_coeff[ed_ctx->txb_itr] == TRUE &&
            blk_ptr->skip_mode == FALSE) {
            uint8_t *pred_buffer = pred_samples->buffer_cb + pred_chroma_offset;

            svt_aom_inv_transform_recon8bit(
                ((int32_t *)residual16bit->buffer_cb) + ed_ctx->coded_area_sb_uv,
                pred_buffer,
                pred_samples->stride_cb,
                pred_buffer,
                pred_samples->stride_cb,
        ...
        // Cr
        //**********************************
        pred_chroma_offset = (((pred_samples->org_y + round_origin_y) >> 1) *
                              pred_samples->stride_cr) +
            ((pred_samples->org_x + round_origin_x) >> 1);

        if (ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
                    .v_has_coeff[ed_ctx->txb_itr] == TRUE &&
            blk_ptr->skip_mode == FALSE) {
            uint8_t *pred_buffer = pred_samples->buffer_cr + pred_chroma_offset;

            svt_aom_inv_transform_recon8bit(
                ((int32_t *)residual16bit->buffer_cr) + ed_ctx->coded_area_sb_uv,
                pred_buffer,
                pred_samples->stride_cr,
                pred_buffer,
                pred_samples->stride_cr,
...
void svt_aom_store16bit_input_src(EbPictureBufferDesc *input_sample16bit_buffer,
    ...
    to_ptr   = (uint16_t *)pcs->input_frame16bit->buffer_cb +
        (sb_x + pcs->input_frame16bit->org_x / 2) +
    ...
    to_ptr   = (uint16_t *)pcs->input_frame16bit->buffer_cr +
        (sb_x + pcs->input_frame16bit->org_x / 2) +
        (sb_y + pcs->input_frame16bit->org_y / 2) * pcs->input_frame16bit->stride_cb;
...
static void perform_intra_coding_loop(PictureControlSet *pcs, SuperBlock *sb_ptr, uint32_t sb_addr,
    ...
    for (ed_ctx->txb_itr = 0; ed_ctx->txb_itr < tot_tu; ed_ctx->txb_itr++) {
        ...
        if (is_16bit) {
            ...
            TxSize tx_size = ed_ctx->blk_geom->txsize[blk_ptr->tx_depth][ed_ctx->txb_itr];
        ...
        ed_ctx->coded_area_sb += ed_ctx->blk_geom->tx_width[blk_ptr->tx_depth][ed_ctx->txb_itr] *
            ed_ctx->blk_geom->tx_height[blk_ptr->tx_depth][ed_ctx->txb_itr];

        // Update the luma Dc Sign Level Coeff Neighbor Array
        {
            uint8_t dc_sign_level_coeff = (uint8_t)ed_ctx->md_ctx
    ...
    if (ed_ctx->blk_geom->has_uv) {
        ...
        svt_aom_get_txb_ctx(pcs,
                            ...
                            blk_originy_uv,
                            ed_ctx->blk_geom->bsize_uv,
                            ed_ctx->blk_geom->txsize_uv[ed_ctx->blk_ptr->tx_depth][ed_ctx->txb_itr],
                            &ed_ctx->md_ctx->cr_txb_skip_context,
                            &ed_ctx->md_ctx->cr_dc_sign_context);
        ...
        if (is_16bit) {
            ...
            for (int32_t plane = 1; plane <= plane_end; ++plane) {
                ...
                svt_av1_predict_intra_block_16bit(
                    ...
                    plane ? 0
                          : (ed_ctx->blk_geom
                                 ->tx_org_x[is_inter][blk_ptr->tx_depth][ed_ctx->txb_itr] -
                             ed_ctx->blk_geom->org_x) >>
                            2,
                    plane ? 0
                          : (ed_ctx->blk_geom
                                 ->tx_org_y[is_inter][blk_ptr->tx_depth][ed_ctx->txb_itr] -
                             ed_ctx->blk_geom->org_y) >>
                            2,
                    plane,
                    ed_ctx->blk_geom->bsize,
    ...
    for (ed_ctx->txb_itr = 0; ed_ctx->txb_itr < tot_tu; ed_ctx->txb_itr++) {
        ...
        if (ed_ctx->blk_geom->has_uv && uv_pass) {
            blk_ptr->block_has_coeff = blk_ptr->block_has_coeff |
                ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
                    .y_has_coeff[ed_ctx->txb_itr] |
                ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
                    .u_has_coeff[ed_ctx->txb_itr] |
                ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
                    .v_has_coeff[ed_ctx->txb_itr];

            if (ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
                    .u_has_coeff[ed_ctx->txb_itr])
                ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds].u_has_coeff[0] =
                    TRUE;
            if (ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
                    .v_has_coeff[ed_ctx->txb_itr])
                ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds].v_has_coeff[0] =
                    TRUE;
        } else {
            blk_ptr->block_has_coeff = blk_ptr->block_has_coeff |
                ed_ctx->md_ctx->md_local_blk_unit[ed_ctx->blk_geom->blkidx_mds]
...
static void av1_copy_frame_mvs(PictureControlSet *pcs, const Av1Common *const cm, MbModeInfo mi,
    ...
    if (blk_ptr->use_intrabc) {
...

        {
            ref_pic_list0 = ref_idx_l0 >= 0
                ? svt_aom_get_ref_pic_buffer(pcs, 1, list_idx0, ref_idx_l0)
                : (EbPictureBufferDesc *)NULL;
            ref_pic_list1 = ref_idx_l1 >= 0
                ? svt_aom_get_ref_pic_buffer(pcs, 1, list_idx1, ref_idx_l1)
                : (EbPictureBufferDesc *)NULL;
        }
    }
...
static void prepare_input_picture(SequenceControlSet *scs, PictureControlSet *pcs,
    ...
    if (is_16bit && scs->static_config.encoder_bit_depth > EB_EIGHT_BIT) {
        ...
        const uint32_t input_cb_offset = (((sb_org_y + input_pic->org_y) >> 1) *
                                          input_pic->stride_cb) +
            ((sb_org_x + input_pic->org_x) >> 1);
        const uint32_t input_cr_offset = (((sb_org_y + input_pic->org_y) >> 1) *
                                          input_pic->stride_cr) +
            ((sb_org_x + input_pic->org_x) >> 1);
...
*
*******************************************/
EB_EXTERN void svt_aom_encode_decode(SequenceControlSet *scs, PictureControlSet *pcs,
    ...
    while (blk_it < scs->max_block_cnt) {
        ...
        if (!pcs->ppcs->sb_geom[sb_addr].block_is_allowed[blk_it] ||
...
*/
EB_EXTERN EbErrorType svt_aom_encdec_update(SequenceControlSet *scs, PictureControlSet *pcs,
                                            SuperBlock *sb_ptr, uint32_t sb_addr, uint32_t sb_org_x,
                                            uint32_t sb_org_y, EncDecContext *ctx) {
    Bool                 is_16bit = ctx->is_16bit;
    EbPictureBufferDesc *recon_buffer;
    ...
    while (blk_it < scs->max_block_cnt) {
        ...
        if (blk_it == 0 && sb_org_x == 0 && blk_geom->org_x == 0 && sb_org_y == 0 &&
            blk_geom->org_y == 0) {
        ...
        for (uint32_t d1_itr = d1_start_blk; d1_itr < (d1_start_blk + num_d1_block); d1_itr++) {
            ...
            pcs->sb_count_nz_coeffs[sb_addr] +=
                md_ctx->md_local_blk_unit[blk_ptr->mds_idx].cnt_nz_coeff;
            ...
            if (md_ctx->bypass_encdec && !(md_ctx->pred_depth_only && md_ctx->md_disallow_nsq)) {
                if (md_ctx->encoder_bit_depth > EB_EIGHT_BIT) {
                    uint32_t recon_luma_offset = (recon_buffer->org_y + ctx->blk_org_y) *
                            recon_buffer->stride_y +
            ...
            if (pcs->cdf_ctrl.update_coef ||
                ...
                EbPictureBufferDesc *coeff_buffer_sb =
                    pcs->ppcs->enc_dec_ptr->quantized_coeff[sb_addr];
                ...
                for (uint16_t tu_it = 0; tu_it < tot_tu; tu_it++) {
                    ...
                    if (pcs->cdf_ctrl.update_coef) {
                        ...
                        ModeDecisionCandidateBuffer **cand_bf_ptr_array_base =
                            md_ctx->cand_bf_ptr_array;
                        ModeDecisionCandidateBuffer **cand_bf_ptr_array = &(
                            cand_bf_ptr_array_base[0]);
            ...
            if (pcs->cdf_ctrl.update_se) {
                ...
                blk_ptr->av1xd->tile_ctx = &pcs->ec_ctx_array[sb_addr];
                svt_aom_update_stats(
                    pcs, blk_ptr, ctx->blk_org_y >> MI_SIZE_LOG2, ctx->blk_org_x >> MI_SIZE_LOG2);
            }
            ...
            // ENCDEC palette info buffer
            {
                if (svt_av1_allow_palette(pcs->ppcs->palette_level, blk_geom->bsize))
                    ...
                    svt_aom_rtime_alloc_palette_info(&sb_ptr->final_blk_arr[final_blk_itr]);
                else
