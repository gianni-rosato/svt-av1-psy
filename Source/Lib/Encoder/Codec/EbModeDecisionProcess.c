...
EbErrorType svt_aom_mode_decision_context_ctor(ModeDecisionContext *ctx, EbColorFormat color_format,
    ...
    if (ctx->hbd_md > EB_8_BIT_MD) {
        EB_MALLOC_ARRAY(ctx->md_local_blk_unit[0].neigh_left_recon_16bit[0],
                        block_max_count_sb * sb_size * sz);
        EB_MALLOC_ARRAY(ctx->md_local_blk_unit[0].neigh_top_recon_16bit[0],
                        block_max_count_sb * sb_size * sz);
        EB_MALLOC_ARRAY(ctx->md_local_blk_unit[0].neigh_left_recon_16bit[1],
                        block_max_count_sb * sb_size * sz >> 1);
        EB_MALLOC_ARRAY(ctx->md_local_blk_unit[0].neigh_top_recon_16bit[1],
                        block_max_count_sb * sb_size * sz >> 1);
        EB_MALLOC_ARRAY(ctx->md_local_blk_unit[0].neigh_left_recon_16bit[2],
                        block_max_count_sb * sb_size * sz >> 1);
        EB_MALLOC_ARRAY(ctx->md_local_blk_unit[0].neigh_top_recon_16bit[2],
                        block_max_count_sb * sb_size * sz >> 1);
    }
    if (ctx->hbd_md != EB_10_BIT_MD) {
        EB_MALLOC_ARRAY(ctx->md_local_blk_unit[0].neigh_left_recon[0],
                        block_max_count_sb * sb_size);
        EB_MALLOC_ARRAY(ctx->md_local_blk_unit[0].neigh_top_recon[0], block_max_count_sb * sb_size);
        EB_MALLOC_ARRAY(ctx->md_local_blk_unit[0].neigh_left_recon[1],
                        block_max_count_sb * sb_size >> 1);
        EB_MALLOC_ARRAY(ctx->md_local_blk_unit[0].neigh_top_recon[1],
                        block_max_count_sb * sb_size >> 1);
        EB_MALLOC_ARRAY(ctx->md_local_blk_unit[0].neigh_left_recon[2],
                        block_max_count_sb * sb_size >> 1);
        EB_MALLOC_ARRAY(ctx->md_local_blk_unit[0].neigh_top_recon[2],
                        block_max_count_sb * sb_size >> 1);
    }
    ...
    for (coded_leaf_index = 0; coded_leaf_index < block_max_count_sb; ++coded_leaf_index) {
        ...
        ctx->md_local_blk_unit[coded_leaf_index].neigh_left_recon[0] =
            ctx->md_local_blk_unit[0].neigh_left_recon[0] + offset;
        ctx->md_local_blk_unit[coded_leaf_index].neigh_top_recon[0] =
            ctx->md_local_blk_unit[0].neigh_top_recon[0] + offset;
        offset >>= 1;
        ctx->md_local_blk_unit[coded_leaf_index].neigh_left_recon[1] =
            ctx->md_local_blk_unit[0].neigh_left_recon[1] + offset;
        ctx->md_local_blk_unit[coded_leaf_index].neigh_top_recon[1] =
            ctx->md_local_blk_unit[0].neigh_top_recon[1] + offset;
        ctx->md_local_blk_unit[coded_leaf_index].neigh_left_recon[2] =
            ctx->md_local_blk_unit[0].neigh_left_recon[2] + offset;
        ctx->md_local_blk_unit[coded_leaf_index].neigh_top_recon[2] =
            ctx->md_local_blk_unit[0].neigh_top_recon[2] + offset;
    }
    ...
    for (coded_leaf_index = 0; coded_leaf_index < block_max_count_sb; ++coded_leaf_index) {
        ...
        if (svt_aom_get_bypass_encdec(enc_mode, ctx->hbd_md, encoder_bit_depth)) {
            ...
            EB_NEW(ctx->md_blk_arr_nsq[coded_leaf_index].coeff_tmp,
            ...
            EB_NEW(ctx->md_blk_arr_nsq[coded_leaf_index].recon_tmp,
                   svt_picture_buffer_desc_ctor,
                   (EbPtr)&init_data);
        } else {
    ...
    EB_NEW(ctx->tx_coeffs,
           svt_picture_buffer_desc_ctor,
           (EbPtr)&thirty_two_width_picture_buffer_desc_init_data);
    EB_NEW(ctx->scratch_prediction_ptr,
           svt_picture_buffer_desc_ctor,
           (EbPtr)&picture_buffer_desc_init_data);
    ...
    EB_NEW(
        ctx->temp_recon_ptr, svt_picture_buffer_desc_ctor, (EbPtr)&picture_buffer_desc_init_data);
    EB_NEW(ctx->temp_residual,
           svt_picture_buffer_desc_ctor,
