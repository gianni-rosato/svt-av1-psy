...
***************************************************/
void mode_decision_update_neighbor_arrays_light_pd0(ModeDecisionContext *ctx) {
...
* Update Recon Samples Neighbor Arrays
***************************************************/
void mode_decision_update_neighbor_arrays(PictureControlSet *pcs, ModeDecisionContext *ctx,
                                          uint32_t index_mds) {
...

    {
        if (!(ctx->pd_pass == PD_PASS_1 && ctx->pic_pred_depth_only)) {
            ...
            neighbor_array_unit_mode_write(ctx->leaf_partition_neighbor_array,
        ...
        if (ctx->rate_est_ctrls.update_skip_coeff_ctx) {
            ...
            neighbor_array_unit_mode_write(ctx->skip_coeff_neighbor_array,
                                           &is_skip_coeff,
                                           org_x,
                                           org_y,
                                           bwdith,
        ...
        if (ctx->rate_est_ctrls.update_skip_ctx_dc_sign_ctx) {
            ...
            for (uint8_t txb_itr = 0; txb_itr < txb_count; txb_itr++) {
                ...
                neighbor_array_unit_mode_write(
                    pcs->md_tx_depth_1_luma_dc_sign_level_coeff_neighbor_array
    ...
    if (ctx->rate_est_ctrls.update_skip_ctx_dc_sign_ctx)
        if (ctx->blk_geom->has_uv && ctx->uv_ctrls.uv_mode <= CHROMA_MODE_1) {
            //  Update chroma CB cbf and Dc context
            {
                ...
                neighbor_array_unit_mode_write(ctx->cb_dc_sign_level_coeff_neighbor_array,
                                               (uint8_t *)&dc_sign_level_coeff,
                                               blk_origin_x_uv,
                                               blk_origin_y_uv,
                                               bwdith_uv,
            ...
            //  Update chroma CR cbf and Dc context
            {
                ...
                neighbor_array_unit_mode_write(ctx->cr_dc_sign_level_coeff_neighbor_array,
                                               (uint8_t *)&dc_sign_level_coeff,
                                               blk_origin_x_uv,
                                               blk_origin_y_uv,
                                               bwdith_uv,
    ...
    if (!ctx->skip_intra) {
        if (ctx->encoder_bit_depth > EB_EIGHT_BIT && ctx->bypass_encdec && !ctx->hbd_md &&
            ...
            if (ctx->txs_ctrls.enabled) {
                update_recon_neighbor_array16bit(
                    pcs->md_tx_depth_1_luma_recon_neighbor_array16bit[MD_NEIGHBOR_ARRAY_INDEX]
                                                                     [tile_idx],
                    ctx->md_local_blk_unit[ctx->blk_geom->blkidx_mds].neigh_top_recon_16bit[0],
                    ctx->md_local_blk_unit[ctx->blk_geom->blkidx_mds].neigh_left_recon_16bit[0],
                    org_x,
                    ...
                    ctx->blk_geom->bheight);
                update_recon_neighbor_array16bit(
                    pcs->md_tx_depth_2_luma_recon_neighbor_array16bit[MD_NEIGHBOR_ARRAY_INDEX]
                                                                     [tile_idx],
                    ctx->md_local_blk_unit[ctx->blk_geom->blkidx_mds].neigh_top_recon_16bit[0],
                    ctx->md_local_blk_unit[ctx->blk_geom->blkidx_mds].neigh_left_recon_16bit[0],
                    org_x,
            ...
            if (ctx->txs_ctrls.enabled) {
                update_recon_neighbor_array(
                    pcs->md_tx_depth_1_luma_recon_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX][tile_idx],
                    ctx->md_local_blk_unit[ctx->blk_geom->blkidx_mds].neigh_top_recon[0],
                    ctx->md_local_blk_unit[ctx->blk_geom->blkidx_mds].neigh_left_recon[0],
                    org_x,
                    org_y,
                    ...
                    ctx->blk_geom->bheight);
                update_recon_neighbor_array(
                    pcs->md_tx_depth_2_luma_recon_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX][tile_idx],
                    ctx->md_local_blk_unit[ctx->blk_geom->blkidx_mds].neigh_top_recon[0],
                    ctx->md_local_blk_unit[ctx->blk_geom->blkidx_mds].neigh_left_recon[0],
                    org_x,
                    org_y,
...
void copy_neighbour_arrays_light_pd0(PictureControlSet *pcs, ModeDecisionContext *ctx,
    ...
    if (!ctx->hbd_md) {
        copy_neigh_arr(pcs->md_luma_recon_neighbor_array[src_idx][tile_idx],
                       pcs->md_luma_recon_neighbor_array[dst_idx][tile_idx],
                       blk_org_x,
                       blk_org_y,
                       blk_geom->bwidth,
                       blk_geom->bheight,
        ...
        copy_neigh_arr(pcs->md_luma_recon_neighbor_array16bit[src_idx][tile_idx],
                       pcs->md_luma_recon_neighbor_array16bit[dst_idx][tile_idx],
                       blk_org_x,
                       blk_org_y,
                       blk_geom->bwidth,
                       blk_geom->bheight,
...
void copy_neighbour_arrays(PictureControlSet *pcs, ModeDecisionContext *ctx, uint32_t src_idx,
    ...
    copy_neigh_arr(pcs->md_intra_luma_mode_neighbor_array[src_idx][tile_idx],
                   pcs->md_intra_luma_mode_neighbor_array[dst_idx][tile_idx],
                   blk_org_x,
                   blk_org_y,
                   blk_geom->bwidth,
                   blk_geom->bheight,
                   NEIGHBOR_ARRAY_UNIT_TOP_AND_LEFT_ONLY_MASK);
    //neighbor_array_unit_reset(pcs->md_skip_flag_neighbor_array[depth]);
    copy_neigh_arr(pcs->md_skip_flag_neighbor_array[src_idx][tile_idx],
                   pcs->md_skip_flag_neighbor_array[dst_idx][tile_idx],
                   blk_org_x,
                   blk_org_y,
                   blk_geom->bwidth,
                   blk_geom->bheight,
    ...
    //neighbor_array_unit_reset(pcs->md_mode_type_neighbor_array[depth]);
    copy_neigh_arr(pcs->md_mode_type_neighbor_array[src_idx][tile_idx],
                   pcs->md_mode_type_neighbor_array[dst_idx][tile_idx],
                   blk_org_x,
                   blk_org_y,
                   blk_geom->bwidth,
                   blk_geom->bheight,
    ...
    //neighbor_array_unit_reset(pcs->md_leaf_depth_neighbor_array[depth]);
    copy_neigh_arr(pcs->mdleaf_partition_neighbor_array[src_idx][tile_idx],
                   pcs->mdleaf_partition_neighbor_array[dst_idx][tile_idx],
                   blk_org_x,
                   blk_org_y,
                   blk_geom->bwidth,
                   blk_geom->bheight,
    ...
    if (!ctx->hbd_md) {
        copy_neigh_arr(pcs->md_luma_recon_neighbor_array[src_idx][tile_idx],
                       ...
                       NEIGHBOR_ARRAY_UNIT_FULL_MASK);
        if (ctx->txs_ctrls.enabled) {
            copy_neigh_arr(pcs->md_tx_depth_1_luma_recon_neighbor_array[src_idx][tile_idx],
                           pcs->md_tx_depth_1_luma_recon_neighbor_array[dst_idx][tile_idx],
                           blk_org_x,
                           blk_org_y,
                           blk_geom->bwidth,
                           blk_geom->bheight,
                           NEIGHBOR_ARRAY_UNIT_FULL_MASK);
            copy_neigh_arr(pcs->md_tx_depth_2_luma_recon_neighbor_array[src_idx][tile_idx],
                           pcs->md_tx_depth_2_luma_recon_neighbor_array[dst_idx][tile_idx],
                           blk_org_x,
                           blk_org_y,
                           blk_geom->bwidth,
                           blk_geom->bheight,
        ...
        if (blk_geom->has_uv && ctx->uv_ctrls.uv_mode <= CHROMA_MODE_1) {
            copy_neigh_arr(pcs->md_cb_recon_neighbor_array[src_idx][tile_idx],
                           pcs->md_cb_recon_neighbor_array[dst_idx][tile_idx],
                           blk_org_x_uv,
                           blk_org_y_uv,
                           bwidth_uv,
                           bheight_uv,
            ...
            copy_neigh_arr(pcs->md_cr_recon_neighbor_array[src_idx][tile_idx],
                           pcs->md_cr_recon_neighbor_array[dst_idx][tile_idx],
                           blk_org_x_uv,
                           blk_org_y_uv,
                           bwidth_uv,
                           bheight_uv,
        ...
        copy_neigh_arr(pcs->md_luma_recon_neighbor_array16bit[src_idx][tile_idx],
                       ...
                       NEIGHBOR_ARRAY_UNIT_FULL_MASK);
        if (ctx->txs_ctrls.enabled) {
            copy_neigh_arr(pcs->md_tx_depth_1_luma_recon_neighbor_array16bit[src_idx][tile_idx],
                           pcs->md_tx_depth_1_luma_recon_neighbor_array16bit[dst_idx][tile_idx],
                           blk_org_x,
                           blk_org_y,
                           blk_geom->bwidth,
                           blk_geom->bheight,
                           NEIGHBOR_ARRAY_UNIT_FULL_MASK);
            copy_neigh_arr(pcs->md_tx_depth_2_luma_recon_neighbor_array16bit[src_idx][tile_idx],
                           pcs->md_tx_depth_2_luma_recon_neighbor_array16bit[dst_idx][tile_idx],
                           blk_org_x,
                           blk_org_y,
                           blk_geom->bwidth,
                           blk_geom->bheight,
        ...
        if (blk_geom->has_uv && ctx->uv_ctrls.uv_mode <= CHROMA_MODE_1) {
            copy_neigh_arr(pcs->md_cb_recon_neighbor_array16bit[src_idx][tile_idx],
                           pcs->md_cb_recon_neighbor_array16bit[dst_idx][tile_idx],
                           blk_org_x_uv,
                           blk_org_y_uv,
                           bwidth_uv,
                           bheight_uv,
            ...
            copy_neigh_arr(pcs->md_cr_recon_neighbor_array16bit[src_idx][tile_idx],
                           pcs->md_cr_recon_neighbor_array16bit[dst_idx][tile_idx],
                           blk_org_x_uv,
                           blk_org_y_uv,
                           bwidth_uv,
                           bheight_uv,
    ...
    //neighbor_array_unit_reset(pcs->md_y_dcs_na[depth]);
    copy_neigh_arr(pcs->md_y_dcs_na[src_idx][tile_idx],
    ...
    copy_neigh_arr(pcs->md_tx_depth_1_luma_dc_sign_level_coeff_neighbor_array[src_idx][tile_idx],
                   pcs->md_tx_depth_1_luma_dc_sign_level_coeff_neighbor_array[dst_idx][tile_idx],
                   blk_org_x,
                   blk_org_y,
                   blk_geom->bwidth,
                   blk_geom->bheight,
                   NEIGHBOR_ARRAY_UNIT_TOP_AND_LEFT_ONLY_MASK);
    if (blk_geom->has_uv && ctx->uv_ctrls.uv_mode <= CHROMA_MODE_1) {
        copy_neigh_arr(pcs->md_cb_dc_sign_level_coeff_neighbor_array[src_idx][tile_idx],
                       pcs->md_cb_dc_sign_level_coeff_neighbor_array[dst_idx][tile_idx],
                       blk_org_x_uv,
                       blk_org_y_uv,
                       bwidth_uv,
                       bheight_uv,
                       NEIGHBOR_ARRAY_UNIT_TOP_AND_LEFT_ONLY_MASK);
        //neighbor_array_unit_reset(pcs->md_cr_dc_sign_level_coeff_neighbor_array[depth]);

        copy_neigh_arr(pcs->md_cr_dc_sign_level_coeff_neighbor_array[src_idx][tile_idx],
                       pcs->md_cr_dc_sign_level_coeff_neighbor_array[dst_idx][tile_idx],
                       blk_org_x_uv,
                       blk_org_y_uv,
                       bwidth_uv,
                       bheight_uv,
    ...
    //neighbor_array_unit_reset(pcs->md_txfm_context_array[depth]);
    copy_neigh_arr(pcs->md_txfm_context_array[src_idx][tile_idx],
                   ...
                   NEIGHBOR_ARRAY_UNIT_TOP_AND_LEFT_ONLY_MASK);
    if (ctx->rate_est_ctrls.update_skip_coeff_ctx) {
        copy_neigh_arr(pcs->md_skip_coeff_neighbor_array[src_idx][tile_idx],
                       pcs->md_skip_coeff_neighbor_array[dst_idx][tile_idx],
                       blk_org_x,
                       blk_org_y,
                       blk_geom->bwidth,
                       blk_geom->bheight,
    ...
    //neighbor_array_unit_reset(pcs->md_ref_frame_type_neighbor_array[depth]);
    copy_neigh_arr(pcs->md_ref_frame_type_neighbor_array[src_idx][tile_idx],
                   pcs->md_ref_frame_type_neighbor_array[dst_idx][tile_idx],
                   blk_org_x,
                   blk_org_y,
                   blk_geom->bwidth,
                   blk_geom->bheight,
    ...
    copy_neigh_arr_32(pcs->md_interpolation_type_neighbor_array[src_idx][tile_idx],
                      pcs->md_interpolation_type_neighbor_array[dst_idx][tile_idx],
                      blk_org_x,
                      blk_org_y,
                      blk_geom->bwidth,
                      blk_geom->bheight,
...
void md_update_all_neighbour_arrays(PictureControlSet *pcs, ModeDecisionContext *ctx,
    ...
    if (avail_blk_flag) {
        ...
        if (!ctx->shut_fast_rate || ctx->rate_est_ctrls.update_skip_ctx_dc_sign_ctx ||
            ...
            update_mi_map(ctx->blk_ptr, ctx->blk_org_x, ctx->blk_org_y, ctx->blk_geom, pcs);
    }
}

void md_update_all_neighbour_arrays_multiple(PictureControlSet *pcs, ModeDecisionContext *ctx,
                                             uint32_t blk_mds, uint32_t sb_origin_x,
                                             uint32_t sb_origin_y) {
...
void av1_perform_inverse_transform_recon_luma(ModeDecisionContext         *ctx,
    ...
    do {
        ...
        if (y_has_coeff)
            ...
            if (ctx->hbd_md) {
                pic_copy_kernel_16bit(((uint16_t *)cand_bf->pred->buffer_y) + txb_origin_index,
                                      cand_bf->pred->stride_y,
                ...
                pic_copy_kernel_8bit(&(cand_bf->pred->buffer_y[txb_origin_index]),
                                     cand_bf->pred->stride_y,
...
void av1_perform_inverse_transform_recon(PictureControlSet *pcs, ModeDecisionContext *ctx,
                                         ModeDecisionCandidateBuffer *cand_bf,
    ...
    do {
        ...
        if (ctx->bypass_encdec && ctx->pd_pass == PD_PASS_1) {
            if (ctx->pred_depth_only && ctx->md_disallow_nsq) {
                get_recon_pic(pcs, &recon_buffer, ctx->hbd_md);
        ...
        if (ctx->md_local_blk_unit[ctx->blk_geom->blkidx_mds].y_has_coeff[txb_itr]) {
            inv_transform_recon_wrapper(cand_bf->pred->buffer_y,
                                        txb_origin_index,
...
*******************************************/
void product_coding_loop_init_fast_loop(PictureControlSet *pcs, ModeDecisionContext *ctx,
                                        NeighborArrayUnit *skip_coeff_neighbor_array,
                                        NeighborArrayUnit *leaf_partition_neighbor_array) {
    ctx->tx_depth = ctx->blk_ptr->tx_depth = 0;
    // Generate Split, Skip and intra mode contexts for the rate estimation
    coding_loop_context_generation(pcs,
                                   ctx,
                                   ctx->blk_ptr,
                                   ctx->blk_org_x,
                                   ctx->blk_org_y,
...
/*
*/
void fast_loop_core_light_pd0(ModeDecisionCandidateBuffer *cand_bf, PictureControlSet *pcs,
    ...
    if (ctx->lpd0_ctrls.pd0_level == VERY_LIGHT_PD0) {
        ...
        if (list_idx_first == 0) {
            ref_pic = get_ref_pic_buffer(pcs, ctx->hbd_md, 0, ref_idx_first);
            mv_x    = cand->mv[REF_LIST_0].x >> 3;
            mv_y    = cand->mv[REF_LIST_0].y >> 3;
        } else {
            ref_pic = get_ref_pic_buffer(pcs, ctx->hbd_md, 1, ref_idx_first);
        ...
        // -------
        use_scaled_rec_refs_if_needed(pcs, input_pic, ref_obj, &ref_pic, ctx->hbd_md);
...
void fast_loop_core_light_pd1(ModeDecisionCandidateBuffer *cand_bf, PictureControlSet *pcs,
                              ModeDecisionContext *ctx, EbPictureBufferDesc *input_pic,
...
#else
    if (ctx->mds0_best_cost != (uint32_t)~0) {
#endif
...
void set_md_stage_counts(PictureControlSet *pcs, ModeDecisionContext *ctx) {
    ...
    uint8_t pic_type = pcs->slice_type == I_SLICE ? 0
        : pcs->ppcs->is_used_as_reference_flag    ? 1
                                                  : 2;
...
/*
*/
void md_stage_0_light_pd0(PictureControlSet *pcs, ModeDecisionContext *ctx,
...
void md_stage_0_light_pd1(PictureControlSet *pcs, ModeDecisionContext *ctx,
                          ModeDecisionCandidateBuffer **cand_bf_ptr_array_base,
                          ModeDecisionCandidate *fast_cand_array, int32_t fast_cand_idx,
                          EbPictureBufferDesc *input_pic, BlockLocation *loc, BlkStruct *blk_ptr) {
...
void md_stage_0(
    ...
    ModeDecisionCandidateBuffer **cand_bf_ptr_array_base, ModeDecisionCandidate *fast_cand_array,
    int32_t fast_candidate_start_index, int32_t fast_candidate_end_index,
    EbPictureBufferDesc *input_pic, uint32_t input_origin_index, uint32_t input_cb_origin_in_index,
    uint32_t input_cr_origin_in_index, BlkStruct *blk_ptr, uint32_t blk_origin_index,
    uint32_t blk_chroma_origin_index, uint32_t cand_bf_start_index, uint32_t max_buffers,
    ...
    ctx->mds_skip_uv_pred = TRUE;
    // 2nd fast loop: src-to-recon
    highest_cost_index   = cand_bf_start_index;
    ...
    for (uint8_t itr = 0; itr < tot_itr; itr++) {
        for (fast_loop_cand_index = fast_candidate_end_index;
             ...
             fast_loop_cand_index--) {
            if (fast_cand_array[fast_loop_cand_index].cand_class == ctx->target_class) {
                ModeDecisionCandidateBuffer *cand_bf = cand_bf_ptr_array_base[highest_cost_index];
                cand_bf->cand                        = &fast_cand_array[fast_loop_cand_index];
                // Initialize tx_depth
                cand_bf->cand->tx_depth       = 0;
                ...
                if (tot_itr > 1) {
                    if (itr == 0) {
                        ...
                        if (fast_cand_array[fast_loop_cand_index].angle_delta[PLANE_TYPE_Y] != 0 ||
                            fast_cand_array[fast_loop_cand_index].filter_intra_mode !=
                                FILTER_INTRA_MODES)
                            continue;
                    } else if (itr == 1) {
                        ...
                        // Eval the child-angular if the parent-angular is the best out of the regular modes (i.e. itr=0 eval)
                            ...
                            if (best_reg_intra_mode !=
                                fast_cand_array[fast_loop_cand_index].pred_mode) {
                                if ((((regular_intra_cost[fast_cand_array[fast_loop_cand_index]
                                                              .pred_mode] -
                        ...
                        // Eval regular only if itr=0 (i.e. skip angular and skip filter)
                        if (fast_cand_array[fast_loop_cand_index].filter_intra_mode ==
                            FILTER_INTRA_MODES)
                            continue;
                ...
                if (scs->vq_ctrls.sharpness_ctrls.unipred_bias && pcs->ppcs->is_noise_level &&
                    is_inter_singleref_mode(cand_bf->cand->pred_mode)) {
                    *cand_bf->fast_cost = (*cand_bf->fast_cost * uni_psy_bias[pcs->picture_qp]) /
                        100;
                }
                if (*cand_bf->fast_cost < ctx->mds0_best_cost) {
                    ctx->mds0_best_cost  = *cand_bf->fast_cost;
                    ctx->mds0_best_class = fast_cand_array[fast_loop_cand_index].cand_class;
                }
                if (tot_itr > 1) {
                    if (itr == 0) {
                        regular_intra_cost[fast_cand_array[fast_loop_cand_index].pred_mode] =
                            *cand_bf->fast_cost;

                        if (*cand_bf->fast_cost < best_reg_intra_cost) {
                            best_reg_intra_cost = *cand_bf->fast_cost;
                            best_reg_intra_mode = fast_cand_array[fast_loop_cand_index].pred_mode;
                        }
                    }
                }
    ...
    *(cand_bf_ptr_array_base[highest_cost_index]->fast_cost) = (scratch_buffer_pesent_flag)
        ? MAX_CU_COST
        : *(cand_bf_ptr_array_base[highest_cost_index]->fast_cost);
}
...
void md_full_pel_search_large_lbd(MV_COST_PARAMS *mv_cost_params, ModeDecisionContext *ctx,
...
void md_full_pel_search(PictureControlSet *pcs, ModeDecisionContext *ctx,
                        EbPictureBufferDesc *input_pic, EbPictureBufferDesc *ref_pic,
                        ...
                        int16_t *best_mvx, int16_t *best_mvy, uint32_t *best_cost, uint8_t hbd_md) {
    // Mvcost params
...
void derive_me_offsets(const SequenceControlSet *scs, PictureControlSet *pcs,
...
void md_nsq_motion_search(PictureControlSet *pcs, ModeDecisionContext *ctx,
    ...
    EbPictureBufferDesc *ref_pic            = get_ref_pic_buffer(pcs, hbd_md, list_idx, ref_idx);
    // -------
    // Use scaled references if resolution of the reference is different from that of the input
    // -------
    use_scaled_rec_refs_if_needed(pcs, input_pic, ref_obj, &ref_pic, hbd_md);
...
*/
void clip_mv_on_pic_boundary(int32_t blk_org_x, int32_t blk_org_y, int32_t bwidth, int32_t bheight,
 ...
 */
int md_subpel_search(PictureControlSet *pcs, ModeDecisionContext *ctx,
    ...
    // Use scaled references if resolution of the reference is different from that of the input
    // -------
    use_scaled_rec_refs_if_needed(pcs, input_pic, ref_obj, &ref_pic, EB_8_BIT_MD);
...
void read_refine_me_mvs_light_pd1(PictureControlSet *pcs, EbPictureBufferDesc *input_pic,
    ...
    for (uint32_t ref_it = 0; ref_it < pcs->ppcs->tot_ref_frame_types; ++ref_it) {
        ...
        if (rf[1] == NONE_FRAME) {
            ...
            if (is_me_data_present(
                    ctx->me_block_offset, ctx->me_cand_offset, me_results, list_idx, ref_idx)) {
                EbPictureBufferDesc *ref_pic = get_ref_pic_buffer(pcs, 0, list_idx, ref_idx);
                EbReferenceObject   *ref_obj =
                ...
                if (ctx->md_subpel_me_ctrls.enabled && !skip_subpel) {
                    ...
                    ctx->post_subpel_me_mv_cost[list_idx][ref_idx] = (uint32_t)md_subpel_search(
                        ...
                        pcs->ppcs->enhanced_picture_ptr, // 10BIT not supported
                        list_idx,
                        ref_idx,
                        &me_mv_x,
                        &me_mv_y);
...
void read_refine_me_mvs(PictureControlSet *pcs, ModeDecisionContext *ctx,
    ...
    uint8_t hbd_md = EB_8_BIT_MD;
    input_pic      = hbd_md ? pcs->input_frame16bit : pcs->ppcs->enhanced_picture_ptr;
...
*/
void pme_search(PictureControlSet *pcs, ModeDecisionContext *ctx, EbPictureBufferDesc *input_pic) {
    uint8_t hbd_md = EB_8_BIT_MD;
            ...
            if (me_data_present) {
                ...
                if (ctx->md_pme_ctrls.early_check_mv_th_multiplier != MIN_SIGNED_VALUE) {
                    ...
                    for (int8_t mvp_index = 0; mvp_index < ctx->mvp_count[list_idx][ref_idx];
                        ...
                        int mv_th = (((pcs->ppcs->enhanced_picture_ptr->width *
...
void av1_cost_calc_cfl(PictureControlSet *pcs, ModeDecisionCandidateBuffer *cand_bf,
    ...
    for (int32_t plane = 0; plane < CFL_PRED_PLANES; plane++) {
        ...
        for (int32_t i = CFL_SIGN_NEG; i < CFL_SIGNS; i++) {
            ...
            const int32_t alpha_rate =
                ctx->md_rate_estimation_ptr->cfl_alpha_fac_bits[joint_sign][plane][0];
        ...
        for (int32_t pn_sign = CFL_SIGN_NEG; pn_sign < CFL_SIGNS; pn_sign++) {
            ...
            for (int32_t c = 0; c < CFL_ALPHABET_SIZE; c++) {
                ...
                for (int32_t i = 0; i < CFL_SIGNS; i++) {
                    ...
                    const int32_t alpha_rate =
                        ctx->md_rate_estimation_ptr->cfl_alpha_fac_bits[joint_sign][plane][c];
...
void check_best_indepedant_cfl(PictureControlSet *pcs, EbPictureBufferDesc *input_pic,
    ...
    if (cand_bf->cand->intra_chroma_mode == UV_CFL_PRED) {
        chroma_rate += ctx->md_rate_estimation_ptr
...
EbErrorType av1_intra_luma_prediction(ModeDecisionContext *ctx, PictureControlSet *pcs,
    ...
    if (!ctx->hbd_md) {
        ...
        if (cand_bf_ptr->cand->angle_delta[PLANE_TYPE_Y] == 0) {
            ...
            if (txb_origin_x != 0 && intra_size.left) {
                ...
                svt_memcpy(left_neigh_array + 1,
                           ctx->tx_search_luma_recon_neighbor_array->left_array + txb_origin_y,
                           tx_height * multipler);
            }
        ...
        if (txb_origin_y != 0 && txb_origin_x != 0)
            top_neigh_array[0] = left_neigh_array[0] =
                ctx->tx_search_luma_recon_neighbor_array
                    ->top_left_array[ctx->tx_search_luma_recon_neighbor_array->max_pic_h +
                                     txb_origin_x - txb_origin_y];
        svt_av1_predict_intra_block(
            ...
            cand_bf_ptr->cand->angle_delta[PLANE_TYPE_Y],
            cand_bf_ptr->cand->palette_info ? (cand_bf_ptr->cand->palette_size[0] > 0) : 0,
            cand_bf_ptr->cand->palette_info, //ATB MD
            cand_bf_ptr->cand->filter_intra_mode,
            top_neigh_array + 1,
            left_neigh_array + 1,
        ...
        if (cand_bf_ptr->cand->angle_delta[PLANE_TYPE_Y] == 0) {
            ...
            if (txb_origin_y != 0 && intra_size.top)
                svt_memcpy(top_neigh_array + 1,
                           (uint16_t *)(ctx->tx_search_luma_recon_neighbor_array16bit->top_array) +
                               txb_origin_x,
            ...
            if (txb_origin_x != 0 && intra_size.left) {
                uint16_t multipler = (txb_origin_y % sb_size_luma + tx_height * intra_size.left) >
                    ...
                    : intra_size.left;
                svt_memcpy(left_neigh_array + 1,
                           (uint16_t *)(ctx->tx_search_luma_recon_neighbor_array16bit->left_array) +
                               txb_origin_y,
        ...
        if (txb_origin_y != 0 && txb_origin_x != 0)
            top_neigh_array[0] = left_neigh_array[0] =
                ((uint16_t *)(ctx->tx_search_luma_recon_neighbor_array16bit->top_left_array) +
                 ctx->tx_search_luma_recon_neighbor_array16bit->max_pic_h + txb_origin_x -
                 txb_origin_y)[0];
...
void tx_initialize_neighbor_arrays(PictureControlSet *pcs, ModeDecisionContext *ctx,
                                   Bool is_inter) {
    uint16_t tile_idx = ctx->tile_index;
    // Set recon neighbor array to be used @ intra compensation
    if (!is_inter) {
        if (ctx->hbd_md)
            ctx->tx_search_luma_recon_neighbor_array16bit = ctx->tx_depth == 2
                ? pcs->md_tx_depth_2_luma_recon_neighbor_array16bit[MD_NEIGHBOR_ARRAY_INDEX]
                                                                   [tile_idx]
                : ctx->tx_depth == 1
                ? pcs->md_tx_depth_1_luma_recon_neighbor_array16bit[MD_NEIGHBOR_ARRAY_INDEX]
            ...
            ctx->tx_search_luma_recon_neighbor_array = ctx->tx_depth == 2
                ? pcs->md_tx_depth_2_luma_recon_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX][tile_idx]
                : ctx->tx_depth == 1
                ? pcs->md_tx_depth_1_luma_recon_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX][tile_idx]
    ...
    ctx->full_loop_luma_dc_sign_level_coeff_neighbor_array = (ctx->tx_depth)
        ? pcs->md_tx_depth_1_luma_dc_sign_level_coeff_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX]
                                                                    [tile_idx]
    ...
    if (ctx->tx_depth) {
        if (!is_inter)
            tx_search_update_recon_sample_neighbor_array(
                ctx->hbd_md ? ctx->tx_search_luma_recon_neighbor_array16bit
                            : ctx->tx_search_luma_recon_neighbor_array,
                cand_bf->recon,
                ctx->blk_geom->tx_org_x[is_inter][ctx->tx_depth][ctx->txb_itr],
                ctx->blk_geom->tx_org_y[is_inter][ctx->tx_depth][ctx->txb_itr],
...
void tx_reset_neighbor_arrays(PictureControlSet *pcs, ModeDecisionContext *ctx, Bool is_inter,
                              uint8_t tx_depth) {
        ...
        if (!is_inter) {
            if (ctx->hbd_md) {
                ...
                if (tx_depth == 2) {
                    copy_neigh_arr(
                        pcs->md_luma_recon_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX][tile_idx],
                        pcs->md_tx_depth_2_luma_recon_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX]
                                                                    [tile_idx],
                        ctx->sb_origin_x + ctx->blk_geom->org_x,
                        ctx->sb_origin_y + ctx->blk_geom->org_y,
                        ctx->blk_geom->bwidth,
                    ...
                    copy_neigh_arr(
                        pcs->md_luma_recon_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX][tile_idx],
                        pcs->md_tx_depth_1_luma_recon_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX]
                                                                    [tile_idx],
                        ctx->sb_origin_x + ctx->blk_geom->org_x,
                        ctx->sb_origin_y + ctx->blk_geom->org_y,
                        ctx->blk_geom->bwidth,
...
void copy_txt_data(ModeDecisionCandidateBuffer *cand_bf, ModeDecisionContext *ctx,
...
uint8_t get_tx_type_group(ModeDecisionContext *ctx, ModeDecisionCandidateBuffer *cand_bf,
                          Bool only_dct_dct) {
    int tx_group = 1;
    if (!only_dct_dct) {
 ...
 **************
*/
void perform_tx_light_pd0(PictureControlSet *pcs, ModeDecisionContext *ctx,
    ...
    // Y: T Q i_q
    av1_estimate_transform(&(((int16_t *)cand_bf->residual->buffer_y)[txb_origin_index]),
    ...
    av1_quantize_inv_quantize_light(pcs,
                                    transf_coeff,
    ...
    if (ctx->rate_est_ctrls.coeff_rate_est_lvl == 0) {
        ...
        *y_coeff_bits = 6000 + cand_bf->eob[0][0] * 500;
        ...
        av1_txb_estimate_coeff_bits_light_pd0(ctx,
                                              cand_bf,
                                              ctx->txb_1d_offset,
                                              cand_bf->quant,
                                              *y_count_non_zero_coeffs,
...
void tx_type_search(PictureControlSet *pcs, ModeDecisionContext *ctx,
                    ModeDecisionCandidateBuffer *cand_bf, uint32_t qindex,
                    uint8_t tx_search_skip_flag, uint32_t *y_count_non_zero_coeffs,
                    uint64_t *y_coeff_bits, uint64_t *y_full_distortion) {
    EbPictureBufferDesc *input_pic = ctx->hbd_md ? pcs->input_frame16bit
    ...
    if (ctx->rate_est_ctrls.update_skip_ctx_dc_sign_ctx)
        get_txb_ctx(pcs,
                    COMPONENT_LUMA,
                    ctx->full_loop_luma_dc_sign_level_coeff_neighbor_array,
...
uint64_t estimate_tx_size_bits(PictureControlSet *pcsPtr, ModeDecisionContext *ctx,
                               ModeDecisionCandidate *cand, Bool skip_flag, uint32_t blk_org_x,
                               uint32_t blk_org_y, BlkStruct *blk_ptr, const BlockGeom *blk_geom,
    ...
    TxMode       tx_mode = pcsPtr->ppcs->frm_hdr.tx_mode;
...
uint64_t get_tx_size_bits(ModeDecisionCandidateBuffer *candidateBuffer, ModeDecisionContext *ctx,
...
void init_tx_cand_bf(ModeDecisionCandidateBuffer *cand_bf, ModeDecisionContext *ctx,
...
void perform_tx_partitioning(ModeDecisionCandidateBuffer *cand_bf, ModeDecisionContext *ctx,
    ...
    for (ctx->tx_depth = start_tx_depth; ctx->tx_depth <= end_tx_depth; ctx->tx_depth++) {
        ...
        if (end_tx_depth) {
            const uint64_t tx_size_bit = pcs->ppcs->frm_hdr.tx_mode == TX_MODE_SELECT
                ? get_tx_size_bits(tx_cand_bf, ctx, pcs, ctx->tx_depth, block_has_coeff)
                : 0;
...
*/
void perform_dct_dct_tx_light_pd1(PictureControlSet *pcs, ModeDecisionContext *ctx,
    ...
    ctx->three_quad_energy           = 0;
    residual_kernel(input_pic->buffer_y,
    ...
    // Y: T Q i_q
    av1_estimate_transform(&(((int16_t *)cand_bf->residual->buffer_y)[loc->blk_origin_index]),
                           cand_bf->residual->stride_y,
                           &(((int32_t *)ctx->tx_coeffs->buffer_y)[0]),
                           NOT_USED_VALUE,
                           tx_size,
                           ...
                           PLANE_TYPE_Y,
                           pf_shape);
    cand_bf->quantized_dc[0][0] = av1_quantize_inv_quantize(
        pcs,
        ctx,
        &(((int32_t *)ctx->tx_coeffs->buffer_y)[0]),
        NOT_USED_VALUE,
    ...
    picture_full_distortion32_bits_single(&(((int32_t *)ctx->tx_coeffs->buffer_y)[0]),
                                          &(((int32_t *)recon_coeff_ptr->buffer_y)[0]),
                                          txbwidth < 64 ? txbwidth : 32,
                                          bwidth,
                                          bheight,
...
void perform_dct_dct_tx(PictureControlSet *pcs, ModeDecisionContext *ctx,
    ...
    if (!tx_search_skip_flag) {
        ...
        av1_estimate_transform(&(((int16_t *)cand_bf->residual->buffer_y)[txb_origin_index]),
                               ...
                               PLANE_TYPE_Y,
                               pf_shape);
        cand_bf->quantized_dc[0][txb_itr] = av1_quantize_inv_quantize(
            pcs,
            ctx,
            &(((int32_t *)ctx->tx_coeffs->buffer_y)[txb_1d_offset]),
            NOT_USED_VALUE,
...
void full_loop_core_light_pd0(PictureControlSet *pcs, ModeDecisionContext *ctx,
    ...
    av1_full_cost_light_pd0(ctx, cand_bf, y_full_distortion, full_lambda, &y_coeff_bits);
}
/*
...
uint8_t do_md_recon(PictureParentControlSet *pcs, ModeDecisionContext *ctxt) {
    ...
    uint8_t need_md_rec_for_ref        = (pcs->is_used_as_reference_flag ||
                                   pcs->scs->static_config.recon_enabled) &&
...
COMPONENT_TYPE chroma_complexity_check(PictureControlSet *pcs, ModeDecisionContext *ctx,
...
*/
void full_loop_core_light_pd1(PictureControlSet *pcs, BlkStruct *blk_ptr, ModeDecisionContext *ctx,
    ...
    if (perform_chroma) {
        ...
        //CHROMA
        full_loop_chroma_light_pd1(pcs,
                                   ctx,
                                   cand_bf,
                                   input_pic,
                                   loc->input_cb_origin_in_index,
...
void full_loop_core(PictureControlSet *pcs, BlkStruct *blk_ptr, ModeDecisionContext *ctx,
                    ModeDecisionCandidateBuffer *cand_bf, ModeDecisionCandidate *cand,
                    EbPictureBufferDesc *input_pic, uint32_t input_origin_index,
...
void update_intra_chroma_mode(ModeDecisionContext *ctx, ModeDecisionCandidateBuffer *cand_bf,
                              PictureControlSet *pcs) {
    ModeDecisionCandidate *cand = cand_bf->cand;
...
static void md_stage_3_light_pd0(PictureControlSet *pcs, ModeDecisionContext *ctx,
    ...
    assert(IMPLIES(ctx->mds_subres_step == 2, ctx->blk_geom->sq_size >= 16));
    assert(IMPLIES(ctx->mds_subres_step == 1, ctx->blk_geom->sq_size >= 8));
    assert_err(IMPLIES(!ctx->disallow_4x4, ctx->mds_subres_step == 0),
...
void move_blk_data_redund(PictureControlSet *pcs, ModeDecisionContext *ctx, BlkStruct *src_cu,
                          BlkStruct *dst_cu) {
    dst_cu->segment_id = src_cu->segment_id;
    if (svt_av1_allow_palette(pcs->ppcs->palette_level, ctx->blk_geom->bsize)) {
        svt_memcpy(&dst_cu->palette_info->pmi, &src_cu->palette_info->pmi, sizeof(PaletteModeInfo));
...
void check_redundant_block(const BlockGeom *blk_geom, ModeDecisionContext *ctx,
...
*/
void check_similar_block(const BlockGeom *blk_geom, ModeDecisionContext *ctx,
...
void init_chroma_mode(ModeDecisionContext *ctx) {
...
void post_mds0_nic_pruning(PictureControlSet *pcs, ModeDecisionContext *ctx,
...
void post_mds1_nic_pruning(ModeDecisionContext *ctx, uint64_t best_md_stage_cost) {
    for (CandClass cand_class_it = CAND_CLASS_0; cand_class_it < CAND_CLASS_TOTAL;
         cand_class_it++) {
...
void post_mds2_nic_pruning(ModeDecisionContext *ctx, uint64_t best_md_stage_cost) {
 ...
 */
void estimate_ref_frames_num_bits(struct ModeDecisionContext *ctx, PictureControlSet *pcs) {
    ...
    if (frm_hdr->reference_mode == REFERENCE_MODE_SELECT) {
        if (MIN(ctx->blk_geom->bwidth, ctx->blk_geom->bheight) >= 8) {
            ...
            reference_mode_context = av1_get_reference_mode_context_new(ctx->blk_ptr->av1xd);
            comp_inter_fac_bits_uni =
                ctx->md_rate_estimation_ptr->comp_inter_fac_bits[reference_mode_context][0];
            comp_inter_fac_bits_bi =
                ctx->md_rate_estimation_ptr->comp_inter_fac_bits[reference_mode_context][1];
        }
    }
    for (uint32_t ref_it = 0; ref_it < pcs->ppcs->tot_ref_frame_types; ++ref_it) {
        MvReferenceFrame ref_pair = pcs->ppcs->ref_frame_type_arr[ref_it];
        ...
        if (rf[1] == NONE_FRAME) {
            ...
            ctx->estimate_ref_frames_num_bits[ref_pair] = estimate_ref_frame_type_bits(
                                                              ctx, ctx->blk_ptr, ref_pair, 1) +
                comp_inter_fac_bits_bi;
        }
    }
}

void calc_scr_to_recon_dist_per_quadrant(ModeDecisionContext *ctx, EbPictureBufferDesc *input_pic,
    ...
    // if a non-4x4 SQ
    if ((ctx->blk_geom->bwidth == ctx->blk_geom->bheight) && ctx->blk_geom->sq_size > 4) {
        EbPictureBufferDesc *recon_ptr = cand_bf->recon;
...
*/
void md_encode_block_light_pd0(PictureControlSet *pcs, ModeDecisionContext *ctx,
                               EbPictureBufferDesc *input_pic) {
...
void copy_recon_md(PictureControlSet *pcs, ModeDecisionContext *ctx,
    ...
    if (!ctx->blk_geom->has_uv && ctx->cfl_ctrls.enabled) {
        ...
        if (ctx->bypass_encdec && ctx->pd_pass == PD_PASS_1) {
            ...
            if (ctx->pred_depth_only && ctx->md_disallow_nsq) {
                get_recon_pic(pcs, &recon_ptr, ctx->hbd_md);
        ...
        if (ctx->encoder_bit_depth > EB_EIGHT_BIT && ctx->bypass_encdec && !ctx->hbd_md &&
            ...
            // Copy 8bit
            if (ctx->pred_depth_only && ctx->md_disallow_nsq)
                recon_ptr = (pcs->ppcs->is_used_as_reference_flag)
    ...
    if (ctx->encoder_bit_depth > EB_EIGHT_BIT && ctx->bypass_encdec) {
        ...
        dst_ptr_top_left_16bit  = (uint16_t *)(ctx->luma_recon_neighbor_array16bit->left_array +
                                              get_neighbor_array_unit_left_index(
                                                  ctx->luma_recon_neighbor_array16bit, blk_org_y) *
                                                  ctx->luma_recon_neighbor_array16bit->unit_size);
...
void lpd1_tx_shortcut_detector(PictureControlSet *pcs, ModeDecisionContext *ctx,
...
void md_encode_block_light_pd1(PictureControlSet *pcs, ModeDecisionContext *ctx, uint32_t sb_addr,
                               EbPictureBufferDesc *input_pic) {
    ModeDecisionCandidateBuffer **cand_bf_ptr_array_base = ctx->cand_bf_ptr_array;
    ModeDecisionCandidateBuffer **cand_bf_ptr_array;
    const BlockGeom              *blk_geom = ctx->blk_geom;
    ...
    if (!ctx->shut_fast_rate && pcs->slice_type != I_SLICE) {
        generate_av1_mvp_table(ctx,
                               ctx->blk_ptr,
                               ctx->blk_geom,
                               ctx->blk_org_x,
                               ctx->blk_org_y,
                               pcs->ppcs->ref_frame_type_arr,
                               pcs->ppcs->tot_ref_frame_types,
                               pcs);
    }

    product_coding_loop_init_fast_loop(
        pcs, ctx, ctx->skip_coeff_neighbor_array, ctx->leaf_partition_neighbor_array);
    ...
    product_full_mode_decision_light_pd1(ctx, blk_ptr, pcs, sb_addr, cand_bf);
...
void md_encode_block(PictureControlSet *pcs, ModeDecisionContext *ctx, uint32_t sb_addr,
                     EbPictureBufferDesc *input_pic) {
    ModeDecisionCandidateBuffer **cand_bf_ptr_array_base = ctx->cand_bf_ptr_array;
    ModeDecisionCandidateBuffer **cand_bf_ptr_array;
    const BlockGeom              *blk_geom = ctx->blk_geom;
    uint32_t input_cb_origin_in_index = ((ctx->round_origin_y >> 1) + (input_pic->org_y >> 1)) *
            input_pic->stride_cb +
        ((ctx->round_origin_x >> 1) + (input_pic->org_x >> 1));
    uint32_t blk_chroma_origin_index = ROUND_UV(blk_geom->org_x) / 2 +
    ...
    if (ctx->md_inter_intra_level) {
        int allow_ii = is_interintra_allowed_bsize(ctx->blk_geom->bsize);
        if (allow_ii)
    ...
    uint32_t input_origin_index = (ctx->blk_org_y + input_pic->org_y) * input_pic->stride_y +
        (ctx->blk_org_x + input_pic->org_x);
    uint32_t blk_origin_index = blk_geom->org_x + blk_geom->org_y * ctx->sb_size;
    for (cand_class_it = CAND_CLASS_0; cand_class_it < CAND_CLASS_TOTAL; cand_class_it++) {
        ...
        if (ctx->md_stage_0_count[cand_class_it] > 0 && ctx->md_stage_1_count[cand_class_it] > 0) {
            buffer_count_for_curr_class = ctx->md_stage_0_count[cand_class_it] >
                    ctx->md_stage_1_count[cand_class_it]
                ? (ctx->md_stage_1_count[cand_class_it] + 1)
                : ctx->md_stage_1_count[cand_class_it];

            buffer_total_count += buffer_count_for_curr_class;
            assert_err(buffer_total_count <= ctx->max_nics, "not enough cand buffers");
            ...
            md_stage_0(
                ...
                ctx->md_stage_0_count[cand_class_it] >
                    ctx->md_stage_1_count[cand_class_it]); //is there need to max the temp buffer
    ...
    if (ctx->encoder_bit_depth > EB_EIGHT_BIT && ctx->bypass_encdec && !ctx->hbd_md &&
        ...
        input_pic                = pcs->input_frame16bit;
        input_cb_origin_in_index = ((ctx->round_origin_y >> 1) + (input_pic->org_y >> 1)) *
                input_pic->stride_cb +
            ((ctx->round_origin_x >> 1) + (input_pic->org_x >> 1));
        blk_chroma_origin_index = ROUND_UV(blk_geom->org_x) / 2 +
            ROUND_UV(blk_geom->org_y) / 2 * (ctx->sb_size >> 1);
        input_origin_index = (ctx->blk_org_y + input_pic->org_y) * input_pic->stride_y +
            (ctx->blk_org_x + input_pic->org_x);
        blk_origin_index = blk_geom->org_x + blk_geom->org_y * ctx->sb_size;
    }
    // 3rd Full-Loop
    ctx->md_stage = MD_STAGE_3;
    md_stage_3(pcs,
    ...
    // Full Mode Decision (choose the best mode)
    uint32_t                     candidate_index = product_full_mode_decision(ctx,
                                                          blk_ptr,
                                                          pcs,
                                                          sb_addr,
                                                          cand_bf_ptr_array,
...
void update_neighbour_arrays(PictureControlSet *pcs, ModeDecisionContext *ctx) {
    ...
    if (ctx->encoder_bit_depth > EB_EIGHT_BIT && ctx->bypass_encdec && !ctx->hbd_md &&
        ctx->pd_pass == PD_PASS_1) {
        ctx->recon_neigh_y  = pcs->md_luma_recon_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX][tile_idx];
        ctx->recon_neigh_cb = pcs->md_cb_recon_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX][tile_idx];
        ctx->recon_neigh_cr = pcs->md_cr_recon_neighbor_array[MD_NEIGHBOR_ARRAY_INDEX][tile_idx];
...
EB_EXTERN EbErrorType svt_aom_check_high_freq(PictureControlSet *pcs, SuperBlock *sb_ptr,
    ...
    EbPictureBufferDesc *input_pic = pcs->ppcs->enhanced_picture_ptr;
