...
#if TUNE_M8_M12
    } else if ((rtc_tune && pcs->enc_mode <= ENC_M9) || (!rtc_tune && pcs->enc_mode <= ENC_M13)) {
#else
...
void svt_aom_set_tpl_extended_controls(PictureParentControlSet *pcs, uint8_t tpl_level) {
    ...
    case 4:
        ...
        tpl_ctrls->intra_mode_end          = DC_PRED;
#if OPT_TPL
        tpl_ctrls->reduced_tpl_group       = pcs->hierarchical_levels == 5 ? 4 : 3;
#else
...
#if OPT_TPL
        tpl_ctrls->reduced_tpl_group            = pcs->hierarchical_levels == 5
            ? is_islice ? 3 : (resolution <= INPUT_SIZE_480p_RANGE ? 3 : 2)
...
#if OPT_TPL
        tpl_ctrls->reduced_tpl_group = pcs->hierarchical_levels == 5
            ? is_islice ? 2 : (resolution <= INPUT_SIZE_480p_RANGE ? 3 : 1)
            : pcs->hierarchical_levels == 4
            ? is_islice ? 2 : (resolution <= INPUT_SIZE_480p_RANGE ? 2 : 1)
            : is_islice ? 3 : (resolution <= INPUT_SIZE_480p_RANGE ? 2 : 0);
#else
        tpl_ctrls->reduced_tpl_group = pcs->hierarchical_levels == 5
...
#if OPT_SG
static uint8_t svt_aom_get_sg_filter_level(EncMode enc_mode, Bool fast_decode, uint8_t input_resolution, Bool is_base, uint8_t seq_qp) {
#else
static uint8_t svt_aom_get_sg_filter_level(EncMode enc_mode, Bool fast_decode, uint8_t input_resolution, Bool is_base) {
    ...
    if (fast_decode == 0) {
        ...
        if (enc_mode <= ENC_M1)
            ...
            sg_filter_lvl = input_resolution <= INPUT_SIZE_360p_RANGE ? (seq_qp > 54 ? 4 : 0) : 0;
            ...
            sg_filter_lvl = input_resolution <= INPUT_SIZE_360p_RANGE ?
                            (seq_qp > 54 ? 4 : 0)                      : 0;
#endif
#else
#if TUNE_M1_M4
...
#if CLN_IS_REF
#if OPT_DLF_QP
static uint8_t get_dlf_level(EncMode enc_mode, uint8_t is_not_last_layer, uint8_t is_16bit, Bool fast_decode,
    ...
    // Don't disable DLF for low resolutions when fast-decode is used
...
#if OPT_DLF_QP
        {
            if (qp < 30)
                ...
                dlf_level = is_base ? 5 : is_not_last_layer ? 0 : 0;
            else
                dlf_level = is_base ? 3 : is_not_last_layer ? 5 : 0;
        }
#else
#if CLN_IS_REF
            dlf_level = (is_16bit && is_not_last_layer) ? 5 : 0;
#else
...
#if TUNE_4X4
    uint8_t allow_4x4_blocks = !svt_aom_get_disallow_4x4(pcs->enc_mode, pcs->temporal_layer_index == 0, pcs->slice_type == I_SLICE);
#endif
...
#if CLN_IS_REF
    const uint8_t           is_not_last_layer       = pcs->temporal_layer_index < pcs->hierarchical_levels;
#endif
...
#else
            enc_mode, is_not_last_layer, bit_depth > EB_EIGHT_BIT, fast_decode, input_resolution, rtc_tune, sc_class1, pcs->picture_qp, (pcs->temporal_layer_index == 0));
#endif
#else
    ...
    if (scs->seq_header.cdef_level && frm_hdr->allow_intrabc == 0) {
        if (scs->static_config.cdef_level == DEFAULT) {
            if (rtc_tune) {
                ...
                pcs->cdef_level = is_base ? 4 : is_not_last_layer ? 7 : 10;
#endif
#if CLN_IS_REF
#if TUNE_CDEF_M56
#if !TUNE_M6_M7
...
#if CLN_IS_REF
        wn = svt_aom_get_wn_filter_level(enc_mode, input_resolution, is_not_last_layer);
#else
        wn = svt_aom_get_wn_filter_level(enc_mode, input_resolution, is_ref);
#endif
#if OPT_SG
        sg = svt_aom_get_sg_filter_level(enc_mode, fast_decode, input_resolution, is_base, scs->static_config.qp);
#else
...
void svt_aom_set_gm_controls(PictureParentControlSet *pcs, uint8_t gm_level) {
    ...
    case 1:
        ...
        gm_ctrls->pp_enabled    = 0;
#if TUNE_GM_PP
        gm_ctrls->pp_downsample_level          = GM_FULL;
#endif
    ...
    case 6:
        ...
        gm_ctrls->pp_enabled                   = 1;
#if TUNE_GM_PP
        gm_ctrls->pp_downsample_level          = GM_DOWN;
#endif
        gm_ctrls->ref_idx0_only                = 1;
...
static void set_inter_comp_controls(ModeDecisionContext *ctx, PictureControlSet *pcs, uint8_t inter_comp_mode) {
...
#if OPT_CMPOUND
        inter_comp_ctrls->no_cmp_low_cmplx     = 0;
        inter_comp_ctrls->no_diff_nsq          = 0;
        inter_comp_ctrls->no_wdg_flat          = 0;
#endif
    ...
    case 5:
        ...
        inter_comp_ctrls->pred0_to_pred1_mult  = 4;
#if OPT_CMPOUND
        inter_comp_ctrls->no_cmp_low_cmplx     = 1;
        inter_comp_ctrls->no_diff_nsq          = 1;
        inter_comp_ctrls->no_wdg_flat          = 1;
        inter_comp_ctrls->no_dist              = 1;
#else
        inter_comp_ctrls->no_dist              = 1;
#endif
...
#if OPT_SG
        sg = svt_aom_get_sg_filter_level(enc_mode, fast_decode, input_resolution, is_base,qp);
#else
        sg = svt_aom_get_sg_filter_level(enc_mode, fast_decode, input_resolution, is_base);
#endif
        if (sg)
...
#if OPT_SG
    uint8_t sg = svt_aom_get_enable_sg(enc_mode, input_resolution, fast_decode, qp);
#else
...
#if OPT_FI
    if (get_filter_intra_level(enc_mode,0) || get_filter_intra_level(enc_mode, 1))
#else
...
#endif
#if OPT_ME_NSQ_QP
    default: md_nsq_me_ctrls->enabled = 0; break;
#else
    default: assert(0); break;
#endif
    }
...
void svt_aom_md_pme_search_controls(ModeDecisionContext *ctx, uint8_t md_pme_level) {
    ...
    case 7:
        ...
        break;
#if OPT_PME_QP
    default: md_pme_ctrls->enabled = 0; break;
#else
    default: assert(0); break;
#endif
    }
...
void svt_aom_set_txt_controls(ModeDecisionContext *ctx, uint8_t txt_level) {
    ...
    case 12:
        ...
        break;
#if OPT_TXT_QP
    default:  txt_ctrls->enabled = 0; break;
#else
    default: assert(0); break;
#endif
    }
...
static void set_cand_reduction_ctrls(PictureControlSet *pcs, ModeDecisionContext *ctx, uint8_t cand_reduction_level,
    ...
    case 6:
        ...
        // reduce_unipred_candidates
#if CLN_IS_REF
        cand_reduction_ctrls->reduce_unipred_candidates = (pcs->ppcs->temporal_layer_index >= pcs->ppcs->hierarchical_levels ||
#else
...
void svt_aom_set_wm_controls(ModeDecisionContext *ctx, uint8_t wm_level) {
    ...
    case 4:
        ...
        break;
#if OPT_WARP_QP
    default: wm_ctrls->enabled = 0; break;
#else
    default: assert(0); break;
#endif
    }
...
uint8_t svt_aom_set_nic_controls(ModeDecisionContext *ctx, uint8_t nic_level) {
    ...
    case 2:
        ...
        if (nic_pruning_ctrls) {
            ...
            nic_pruning_ctrls->mds2_q_weight            = 10;
            nic_pruning_ctrls->mds3_q_weight            = 10;
#endif
        }
        md_staging_mode = MD_STAGING_MODE_2;
        break;
...
void svt_aom_set_nsq_ctrls(ModeDecisionContext *ctx, uint8_t nsq_level, uint8_t *allow_HVA_HVB, uint8_t *allow_HV4,
...
#if OPT_NSQ_RATE_THS
        nsq_ctrls->rate_th_offset_lte16 = 0;
#endif
        nsq_ctrls->psq_txs_lvl                  = 0;
        nsq_ctrls->sub_depth_block_lvl          = 0;
        nsq_ctrls->psq_pred_lvl                 = 0;
    ...
    case 2:
...
#if OPT_MR_M0
        nsq_ctrls->lower_depth_split_cost_th    = 3;
#else
        ...
        nsq_ctrls->non_HV_split_rate_modulation = 0;
#if OPT_NSQ_RATE_THS
        nsq_ctrls->rate_th_offset_lte16 = 10;
#endif
        nsq_ctrls->psq_txs_lvl                  = 0;
        nsq_ctrls->sub_depth_block_lvl          = 0;
...
static uint64_t compute_intra_pd0_th(SequenceControlSet *scs, PictureControlSet *pcs, ModeDecisionContext *ctx) {
    ...
    if (scs->calculate_variance) {
        ...
        if (pcs->ppcs->variance[ctx->sb_index][ME_TIER_ZERO_PU_64x64] <= var_th0)
            use_intra_pd0_th = RDCOST(fast_lambda, cost_th_rate, sb_size * 3);
        else if (pcs->ppcs->variance[ctx->sb_index][ME_TIER_ZERO_PU_64x64] <= var_th1)
            use_intra_pd0_th = RDCOST(fast_lambda, cost_th_rate, sb_size * 2);
        else
            use_intra_pd0_th = RDCOST(fast_lambda, cost_th_rate, sb_size * 1);
#else
        const uint16_t var_th0 = 400; // low-texture 64x64
        const uint16_t var_th1 = 800; // moderate-texture 64x64
        if (pcs->ppcs->variance[ctx->sb_index][ME_TIER_ZERO_PU_64x64] <= var_th0)
...
#if CLN_IS_REF
    Bool                    is_not_last_layer = ppcs->temporal_layer_index < ppcs->hierarchical_levels;
#endif
...
#if CLN_IS_REF
    if (tx_shortcut_level && (ppcs->temporal_layer_index < ppcs->hierarchical_levels) && pcs->enc_mode <= ENC_M12)
#else
...
#if IMPROVE_DR_PD1
    {
        uint8_t lpd1_lvl = pcs->pic_lpd1_lvl;
        if (lpd1_lvl && pcs->ppcs->input_resolution >= INPUT_SIZE_480p_RANGE) {
#if OPT_LPD1_DET
            // Checking against 7, as 7 is the max level for lpd1_lvl. If max level for lpd1_lvl changes, the check should be updated
            if (ctx->is_low_mv)
                lpd1_lvl = MAX(0, MIN(lpd1_lvl + 1, 7));
#else
            if (ctx->is_low_mv)
                lpd1_lvl = MAX(0, MIN(lpd1_lvl + 1, 6));
#endif
        }
        set_lpd1_ctrls(ctx,
                       ctx->high_freq_present ? MIN(lpd1_lvl, ctx->detect_high_freq_ctrls.max_pic_lpd1_lvl) : lpd1_lvl);
    }
#else
        set_lpd1_ctrls(ctx,
                       ctx->high_freq_present ? MIN(pcs->pic_lpd1_lvl, ctx->detect_high_freq_ctrls.max_pic_lpd1_lvl)
...
void svt_aom_sig_deriv_enc_dec_light_pd0(SequenceControlSet *scs, PictureControlSet *pcs, ModeDecisionContext *ctx) {
    ...
    const bool               rtc_tune  = (scs->static_config.pred_structure == SVT_AV1_PRED_LOW_DELAY_B) ? true : false;
#if CLN_IS_REF
    const bool               is_not_last_layer = ppcs->temporal_layer_index < ppcs->hierarchical_levels;
#endif
...
void svt_aom_sig_deriv_enc_dec(SequenceControlSet *scs, PictureControlSet *pcs, ModeDecisionContext *ctx) {
...
#endif
#if TUNE_ILV
    const uint8_t seq_qp = scs->static_config.qp;
#endif
...
#if TUNE_SHIFT_PRESETS
        ctx->md_disallow_nsq = enc_mode <= ENC_M1 ? !ctx->nsq_ctrls.enabled : 1;
#else
    ...
    if (pd_pass == PD_PASS_0)
#if TUNE_SHIFT_PRESETS
        ctx->md_subpel_me_level = enc_mode <= ENC_M6 ? 4 : 0;
#else
...
#if NEW_M0
#if TUNE_NEW_M1
        ctx->md_subpel_pme_level = enc_mode <= ENC_M1 ? 3 : 0;
    else if (enc_mode <= ENC_M1)
#else
...
uint8_t svt_aom_get_nsq_level(EncMode enc_mode, uint8_t is_islice, uint8_t is_base, InputCoeffLvl coeff_lvl) {
    ...
    if (enc_mode <= ENC_MR){
        ...
        if (coeff_lvl == LOW_LVL)
            nsq_level = is_base ? 4 : 5;
        else if (coeff_lvl == HIGH_LVL)
            nsq_level = is_base ? 10 : 11;
        else // regular
            nsq_level = is_base ? 4 : 5;
#else
        nsq_level = 2;
...
#endif
        if (coeff_lvl == LOW_LVL)
            nsq_level = is_base ? 4 : 7;
        else if (coeff_lvl == HIGH_LVL)
            nsq_level = is_base ? 10 : 11;
        else // regular
            nsq_level = is_base ? 5 : 8;
    }
#endif
#else
    if (enc_mode <= ENC_M0)
    ...
    if (enc_mode <= ENC_MR) {
        ...
        if (qp <= 44) {
            nsq_level = MIN(nsq_level + 2, 14);
        } else if (qp <= 48) {
            nsq_level = MIN(nsq_level + 1, 14);
        } else if (qp <= 56) {
            nsq_level = nsq_level;
        } else if (qp <= 60) {
            nsq_level = nsq_level - 1;
        } else {
            nsq_level = nsq_level - 2;
        }
#else
        nsq_level = nsq_level;
#endif
...
#if CLN_IS_REF
                                                              Bool is_islice, Bool sc_class1, Bool is_not_last_layer,
#else
...
#endif
            pcs->pic_lpd0_lvl = MIN(8, 1 + ldp0_lvl_offset[qp_band_idx]);
#endif
#if TUNE_M9_LPD0_LVLS
#if TUNE_M10_II
        else if (enc_mode <= ENC_M10) {
#else
        else if (enc_mode <= ENC_M9) {
...
#endif
        filter_intra_level = is_base ? 1 : 0;
#endif
    else
...
void svt_aom_sig_deriv_mode_decision_config(SequenceControlSet *scs, PictureControlSet *pcs) {
    ...
    // for a given picture.
#if OPT_FI
    pcs->pic_filter_intra_level = get_filter_intra_level(enc_mode, is_base);
#else
    ...
    frm_hdr->allow_high_precision_mv = (
        frm_hdr->quantization_params.base_q_idx < HIGH_PRECISION_MV_QTHRESH_0 ||
        (pcs->ref_hp_percentage > HIGH_PRECISION_REF_PERC_TH && frm_hdr->quantization_params.base_q_idx < HIGH_PRECISION_MV_QTHRESH_1))
...
#endif
#if OPT_WARP_QP
    if (scs->qp_tune && pcs->wm_level) {
        int mult = 1;
        if (scs->static_config.qp <= 32)
            pcs->wm_level += 2;
        else if (scs->static_config.qp <= 40)
            pcs->wm_level += 1;
        else if (scs->static_config.qp > 56)
            pcs->wm_level = MAX(1, (int)pcs->wm_level - 1);
    }
#endif
...
#if OPT_OBMC_QP
    if (scs->qp_tune && ppcs->pic_obmc_level) {
        if (scs->static_config.qp <= 32)
            ppcs->pic_obmc_level += 2;
        else if (scs->static_config.qp <= 40)
            ppcs->pic_obmc_level += 1;
        else if (scs->static_config.qp > 56)
            ppcs->pic_obmc_level = MAX(1, (int)ppcs->pic_obmc_level - 1);
    }
#endif
    // Switchable Motion Mode
...
#if OPT_TXT_QP
    if (scs->qp_tune && pcs->txt_level) {
        if (scs->static_config.qp <= 32)
            pcs->txt_level += 3;
        else if (scs->static_config.qp <= 40)
            pcs->txt_level += 1;
        else if (scs->static_config.qp > 56)
            pcs->txt_level = MAX(1, (int)pcs->txt_level - 1);
    }
#endif
#if OPT_TXT_QP_2
    if (enc_mode <= ENC_MR) {
        pcs->txt_level = pcs->txt_level;
    } else if (enc_mode <= ENC_M6) {
        if (scs->static_config.qp <= 44) {
            pcs->txt_level = MIN(pcs->txt_level + 2, 10);
        } else if (scs->static_config.qp <= 48) {
            pcs->txt_level = MIN(pcs->txt_level + 1, 10);
        } else if (scs->static_config.qp <= 56) {
            pcs->txt_level = pcs->txt_level;
        } else if (scs->static_config.qp <= 60) {
            pcs->txt_level = MAX(pcs->txt_level - 1, 1);
        } else {
            pcs->txt_level = MAX(pcs->txt_level - 2, 1);
        }
    }
#endif
    ...
    if (rtc_tune) {
        ...
        if (enc_mode <= ENC_M5)
            pcs->tx_shortcut_level = 0;
        else if ((enc_mode <= ENC_M8 && !pcs->ppcs->sc_class1) || (enc_mode <= ENC_M9 && pcs->ppcs->sc_class1))
        ...
        if (enc_mode <= ENC_M5)
            ...
            pcs->tx_shortcut_level = is_islice ? 0 : 5;
#else
...
#else
        if (enc_mode <= ENC_MR)
            pcs->dist_based_ref_pruning = is_base ? 1 : 3;
        else if (enc_mode <= ENC_M0)
#endif
...
#if OPT_MR_M0
#if OPT_NSQ_QP
    pcs->nsq_level = svt_aom_get_nsq_level(enc_mode, is_base, pcs->coeff_lvl, scs->static_config.qp);
#else
    pcs->nsq_level = svt_aom_get_nsq_level(enc_mode, is_base, pcs->coeff_lvl);
#endif
#else
...
#endif
#if OPT_TXS_QP
    if (scs->qp_tune && pcs->txs_level) {
        if (scs->static_config.qp <= 32)
            pcs->txs_level += 2;
        else if (scs->static_config.qp <= 40)
            pcs->txs_level += 1;
        else if (scs->static_config.qp > 56)
            pcs->txs_level = MAX(1, (int)pcs->txs_level - 1);
    }
#endif
...
#if OPT_NIC_QP
    pcs->nic_level = svt_aom_get_nic_level(enc_mode, is_base, hierarchical_levels, pcs->scs->static_config.qp, rtc_tune);
#else
    ...
    if (enc_mode <= ENC_M0)
        ...
        pcs->md_nsq_mv_search_level = 5;
#else
    pcs->md_nsq_mv_search_level = 4;
#endif
#if OPT_ME_NSQ_QP
    if (scs->qp_tune && pcs->md_nsq_mv_search_level) {
        if (scs->static_config.qp <= 32)
            pcs->md_nsq_mv_search_level += 2;
        else if (scs->static_config.qp <= 40)
            pcs->md_nsq_mv_search_level += 1;
        else if (scs->static_config.qp > 56)
            pcs->md_nsq_mv_search_level = MAX(1, (int)pcs->md_nsq_mv_search_level - 1);
    }
#endif
...
#endif
#if OPT_PME_QP
    if (scs->qp_tune && pcs->md_pme_level) {
        if (scs->static_config.qp <= 32)
            pcs->md_pme_level += 3;
        else if (scs->static_config.qp <= 40)
            pcs->md_pme_level += 1;
        else if (scs->static_config.qp > 56)
            pcs->md_pme_level = MAX(1, (int)pcs->md_pme_level - 1);
    }
#endif
    ...
    */
#if TUNE_4X4
    pcs->pic_disallow_4x4 = svt_aom_get_disallow_4x4(enc_mode, is_base, is_islice);
#else
    ...
    */
    ...
    if ((scs->static_config.encoder_bit_depth == EB_EIGHT_BIT || !pcs->nsq_level) &&
        ...
        pcs->pic_bypass_encdec = svt_aom_get_bypass_encdec(
            enc_mode, scs->static_config.encoder_bit_depth);
#else
        pcs->pic_bypass_encdec = svt_aom_get_bypass_encdec(
    ...
    */
    ...
    pcs->pic_disallow_below_16x16 = svt_aom_get_disallow_below_16x16_picture_level(
        enc_mode, input_resolution, is_islice, ppcs->sc_class1, is_not_last_layer, rtc_tune);
#else
    pcs->pic_disallow_below_16x16 = svt_aom_get_disallow_below_16x16_picture_level(
    ...
    if (ppcs->sc_class1) {
        ...
        if (rtc_tune) {
            ...
            if (pcs->coeff_lvl == LOW_LVL) {
                pcs->pic_block_based_depth_refinement_level = hierarchical_levels == 5 ? (is_base ? 3 : 7)
                                                                                       : (is_base ? 4 : 8);
            } else { // regular
                pcs->pic_block_based_depth_refinement_level = hierarchical_levels == 5 ? (is_base ? 8 : 11)
                                                                                       : (is_base ? 9 : 12);
            }
        }
#endif
    ...
    if (ppcs->sc_class1) {
        ...
        if (pcs->coeff_lvl == LOW_LVL)
            ...
            pcs->pic_block_based_depth_refinement_level = is_base ? 1 : 5;
    } else if (enc_mode <= ENC_M3) {
        if (pcs->coeff_lvl == LOW_LVL)
            pcs->pic_block_based_depth_refinement_level = is_base ? 1 : 2;
        else
        ...
        pcs->pic_block_based_depth_refinement_level = is_base ? 0 : 1;
    else if (enc_mode <= ENC_M3) {
        if (pcs->coeff_lvl == LOW_LVL)
            pcs->pic_block_based_depth_refinement_level = is_base ? 1 : 2;
        else
        ...
        if (rtc_tune) {
            ...
            if (pcs->coeff_lvl == LOW_LVL)
                pcs->pic_block_based_depth_refinement_level = is_base ? 2 : 4;
            else
