    ...
    for (sb_index = 0; sb_index < picture_sb_width * picture_sb_height; ++sb_index) {
        ...
        for (md_scan_block_index = 0; md_scan_block_index < max_block_count; md_scan_block_index++) {
            ...
            if (scs->over_boundary_block_mode == 1) {
                ...
                uint8_t has_rows = (scs->sb_geom[sb_index].org_y + sq_blk_geom->org_y + sq_blk_geom->bheight / 2 < scs->max_input_luma_height);
                uint8_t has_cols = (scs->sb_geom[sb_index].org_x + sq_blk_geom->org_x + sq_blk_geom->bwidth / 2 < scs->max_input_luma_width);
