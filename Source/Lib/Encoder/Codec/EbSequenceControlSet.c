...
extern EbErrorType sb_params_init(SequenceControlSet *scs_ptr) {
    ...
    for (sb_index = 0; sb_index < picture_sb_width * picture_sb_height; ++sb_index) {
        ...
        //                         // 2
        ///////////////////////////    |
        if (scs_ptr->input_resolution <= INPUT_SIZE_576p_RANGE_OR_LOWER) {
            potential_logo_sb =
                (scs_ptr->sb_params_array[sb_index].origin_x >=
                 (scs_ptr->seq_header.max_frame_width - (3 * scs_ptr->sb_sz))) &&
                        (scs_ptr->sb_params_array[sb_index].origin_y < 2 * scs_ptr->sb_sz)
                    ? 1
                    : potential_logo_sb;
            potential_logo_sb =
                (scs_ptr->sb_params_array[sb_index].origin_x < ((3 * scs_ptr->sb_sz))) &&
                        (scs_ptr->sb_params_array[sb_index].origin_y < 2 * scs_ptr->sb_sz)
                    ? 1
                    : potential_logo_sb;
            potential_logo_sb = (scs_ptr->sb_params_array[sb_index].origin_y >=
                                 (scs_ptr->seq_header.max_frame_height - (2 * scs_ptr->sb_sz)))
                                    ? 1
                                    : potential_logo_sb;
        } else if (scs_ptr->input_resolution < INPUT_SIZE_4K_RANGE) {
            potential_logo_sb =
                (scs_ptr->sb_params_array[sb_index].origin_x >=
                 (scs_ptr->seq_header.max_frame_width - (7 * scs_ptr->sb_sz))) &&
                        (scs_ptr->sb_params_array[sb_index].origin_y < 4 * scs_ptr->sb_sz)
                    ? 1
                    : potential_logo_sb;
            potential_logo_sb =
                (scs_ptr->sb_params_array[sb_index].origin_x < ((7 * scs_ptr->sb_sz))) &&
                        (scs_ptr->sb_params_array[sb_index].origin_y < 4 * scs_ptr->sb_sz)
                    ? 1
                    : potential_logo_sb;
            potential_logo_sb = (scs_ptr->sb_params_array[sb_index].origin_y >=
                                 (scs_ptr->seq_header.max_frame_height - (4 * scs_ptr->sb_sz)))
                                    ? 1
                                    : potential_logo_sb;
        } else {
            potential_logo_sb =
                (scs_ptr->sb_params_array[sb_index].origin_x >=
                 (scs_ptr->seq_header.max_frame_width - (14 * scs_ptr->sb_sz))) &&
                        (scs_ptr->sb_params_array[sb_index].origin_y < 8 * scs_ptr->sb_sz)
                    ? 1
                    : potential_logo_sb;
            potential_logo_sb =
                (scs_ptr->sb_params_array[sb_index].origin_x < ((14 * scs_ptr->sb_sz))) &&
                        (scs_ptr->sb_params_array[sb_index].origin_y < 8 * scs_ptr->sb_sz)
                    ? 1
                    : potential_logo_sb;
            potential_logo_sb = (scs_ptr->sb_params_array[sb_index].origin_y >=
                                 (scs_ptr->seq_header.max_frame_height - (8 * scs_ptr->sb_sz)))
                                    ? 1
                                    : potential_logo_sb;
        }
        scs_ptr->sb_params_array[sb_index].potential_logo_sb = potential_logo_sb;
