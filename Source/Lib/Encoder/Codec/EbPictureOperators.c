...
void residual_kernel_subsampled(uint8_t *input, uint32_t input_stride, uint8_t *pred,
    ...
    while (row_index < area_height) {
        ...
        while (column_index < area_width) {
            residual[column_index] = ((int16_t)input[column_index]) - ((int16_t)pred[column_index]);
            residual[column_index + residual_stride] =
                ((int16_t)input[column_index]) - ((int16_t)pred[column_index]);
            ++column_index;
        }
...
uint64_t compute_nxm_satd_8x8_units_u8(
    ...
    for (block_index_in_height = 0; block_index_in_height < (height >> 3);
         ++block_index_in_height) {
        for (block_index_in_width = 0; block_index_in_width < (width >> 3); ++block_index_in_width)
            satd += compute8x8_satd_u8(
                &(src[(block_index_in_width << 3) + (block_index_in_height << 3) * src_stride]),
                dc_value,
                src_stride);
    }
...
uint64_t compute_nxm_satd_4x4_units_u8(
    uint8_t * src, //int16_t *diff,       // input parameter, diff samples Ptr
    uint32_t  src_stride, //uint32_t  diffStride, // input parameter, source stride
    uint32_t  width, // input parameter, block width (N)
    uint32_t  height, // input parameter, block height (M)
    ...
    for (block_index_in_height = 0; block_index_in_height < (height >> 2);
         ++block_index_in_height) {
        for (block_index_in_width = 0; block_index_in_width < (width >> 2); ++block_index_in_width)
            satd += compute4x4_satd_u8(
                &(src[(block_index_in_width << 2) + (block_index_in_height << 2) * src_stride]),
                dc_value,
                src_stride);
    }
 ...
 *******************************************/
uint64_t compute_nx_m_satd_sad_sb(uint8_t *src, // input parameter, source samples Ptr
    ...
    if (width >= 8 && height >= 8) {
        satd = compute_nxm_satd_8x8_units_u8(src, src_stride, width, height, &dc_value);
    } else {
        satd = compute_nxm_satd_4x4_units_u8(src, src_stride, width, height, &dc_value);
    }

    ac_value = satd - (dc_value >> 2);
...
void extract_8bit_data(uint16_t *in16_bit_buffer, uint32_t in_stride, uint8_t *out8_bit_buffer,
    ...
    if (((width & 3) == 0) && ((height & 1) == 0)) {
        un_pack8_bit_data(in16_bit_buffer, in_stride, out8_bit_buffer, out8_stride, width, height);
    } else {
        un_pack8_bit_data_c(
...
void memset16bit_block(int16_t *in_ptr, uint32_t stride_in, uint32_t size, int16_t value) {
    uint32_t i;
    for (i = 0; i < size; i++) memset16bit((uint16_t *)in_ptr + i * stride_in, value, size);
}

static void extend_plane(uint8_t *const src, int32_t src_stride, int32_t width, int32_t height,
                         int32_t extend_top, int32_t extend_left, int32_t extend_bottom,
                         int32_t extend_right) {
    int32_t       i;
...
void eb_aom_yv12_extend_frame_borders_c(Yv12BufferConfig *ybf, const int32_t num_planes) {
...
void eb_aom_yv12_copy_frame_c(const Yv12BufferConfig *src_bc, Yv12BufferConfig *dst_bc,
