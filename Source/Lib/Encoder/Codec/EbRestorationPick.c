...
static void init_rsc(const Yv12BufferConfig *src, Av1Common *cm, const Macroblock *x, int32_t plane,
                     RestUnitSearchInfo *rusi, Yv12BufferConfig *dst, RestSearchCtxt *rsc) {
    rsc->src   = src;
    rsc->dst   = dst;
    rsc->cm    = cm;
    rsc->x     = x;
...
static int64_t try_restoration_unit(const RestSearchCtxt *rsc, const RestorationTileLimits *limits,
...
static void search_sgrproj(const RestorationTileLimits *limits, const Av1PixelRect *tile,
...
static void search_wiener(const RestorationTileLimits *limits, const Av1PixelRect *tile_rect,
                          int32_t rest_unit_idx, void *priv) {
    RestSearchCtxt *       rsc     = (RestSearchCtxt *)priv;
    RestUnitSearchInfo *   rusi    = &rsc->rusi[rest_unit_idx];
    const Av1Common *const cm      = rsc->cm;
    ...
    rusi->sse[RESTORE_WIENER] = finer_tile_search_wiener(rsc, limits, tile_rect, &rui, wiener_win);
...
static void search_norestore(const RestorationTileLimits *limits, const Av1PixelRect *tile_rect,
                             int32_t rest_unit_idx, void *priv) {
    (void)tile_rect;
