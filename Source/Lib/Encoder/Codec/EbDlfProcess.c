 ...
 ******************************************************/
EbErrorType svt_aom_dlf_context_ctor(EbThreadContext *thread_ctx, const EbEncHandle *enc_handle_ptr,
                                     int index) {
    ...
    for (;;) {
        ...
        if (is_16bit && scs->static_config.encoder_bit_depth == EB_EIGHT_BIT) {
            ...
            if (pcs->pic_bypass_encdec) {
                ...
                svt_convert_pic_8bit_to_16bit(recon_pic,
        ...
        if ((dlf_enable_flag && !pcs->ppcs->dlf_ctrls.sb_based_dlf) ||
            ...
            svt_av1_pick_filter_level(
                (EbPictureBufferDesc *)pcs->ppcs->enhanced_pic, pcs, LPF_PICK_FROM_FULL_IMAGE);
        ...
        //pre-cdef prep
        {
            ...
            if (scs->seq_header.cdef_level && pcs->ppcs->cdef_level) {
                ...
                const uint32_t offset_cb = (recon_pic->org_x +
                                            recon_pic->org_y * recon_pic->stride_cb) >>
                    1;
                pcs->cdef_input_recon[1] = recon_pic->buffer_cb + (offset_cb << is_16bit);
                const uint32_t offset_cr = (recon_pic->org_x +
                                            recon_pic->org_y * recon_pic->stride_cr) >>
                    1;
                ...
                const uint32_t       input_offset_y = input_pic->org_x +
                    input_pic->org_y * input_pic->stride_y;
                pcs->cdef_input_source[0]      = input_pic->buffer_y + (input_offset_y << is_16bit);
                const uint32_t input_offset_cb = (input_pic->org_x +
                                                  input_pic->org_y * input_pic->stride_cb) >>
                    1;
                pcs->cdef_input_source[1] = input_pic->buffer_cb + (input_offset_cb << is_16bit);
                const uint32_t input_offset_cr = (input_pic->org_x +
                                                  input_pic->org_y * input_pic->stride_cr) >>
                    1;
        ...
        pcs->cdef_segments_total_count  = (uint16_t)(pcs->cdef_segments_column_count *
                                                    pcs->cdef_segments_row_count);
