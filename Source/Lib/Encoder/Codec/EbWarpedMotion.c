...
int eb_get_shear_params(EbWarpedMotionParams *wm) {
    ...
    wm->delta =
        clamp(mat[5] - (int)ROUND_POWER_OF_TWO_SIGNED_64(v, shift) - (1 << WARPEDMODEL_PREC_BITS),
              INT16_MIN,
              INT16_MAX);
    ...
    wm->beta =
        ROUND_POWER_OF_TWO_SIGNED(wm->beta, WARP_PARAM_REDUCE_BITS) * (1 << WARP_PARAM_REDUCE_BITS);
...
static void highbd_warp_plane(EbWarpedMotionParams *wm, const uint8_t *const ref8, int width,
...
static void warp_plane(EbWarpedMotionParams *wm, const uint8_t *const ref, int width, int height,
