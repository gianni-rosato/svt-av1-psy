...
extern void svt_aom_get_recon_pic(PictureControlSet *pcs, EbPictureBufferDesc **recon_ptr,
    ...
    if (*recon_ptr &&
        (pcs->ppcs->render_width != (*recon_ptr)->width ||
         pcs->ppcs->render_height != (*recon_ptr)->height)) {
...
static EbPictureBufferDesc *get_own_recon(SequenceControlSet *scs, PictureControlSet *pcs,
    ...
    if (is_16bit) {
        ...
        uint16_t            *org_ptr = (uint16_t *)org_rec->buffer_y + org_rec->org_x +
            org_rec->org_y * org_rec->stride_y;
        ...
        EbPictureBufferDesc *org_rec = context_ptr->org_rec_frame;
        uint8_t             *org_ptr = &(
            (org_rec->buffer_y)[org_rec->org_x + org_rec->org_y * org_rec->stride_y]);
        uint8_t *org_ptr_cb = &(
            (org_rec->buffer_cb)[org_rec->org_x / 2 + org_rec->org_y / 2 * org_rec->stride_cb]);
        uint8_t *org_ptr_cr = &(
...
void svt_convert_pic_8bit_to_16bit(EbPictureBufferDesc *src_8bit, EbPictureBufferDesc *dst_16bit,
    ...
    // Y
    buffer_16bit = (uint16_t *)(dst_16bit->buffer_y) + dst_16bit->org_x +
        dst_16bit->org_y * dst_16bit->stride_y;
    ...
    buffer_8bit  = src_8bit->buffer_cb + (src_8bit->org_x >> ss_x) +
        (src_8bit->org_y >> ss_y) * src_8bit->stride_cb;
    ...
    buffer_8bit  = src_8bit->buffer_cr + (src_8bit->org_x >> ss_x) +
        (src_8bit->org_y >> ss_y) * src_8bit->stride_cr;
...
void set_unscaled_input_16bit(PictureControlSet *pcs) {
    ...
    if (input_pic->bit_depth == EB_EIGHT_BIT)
        ...
        uint16_t *planes[3] = {(uint16_t *)output_pic->buffer_y +
                                   (output_pic->org_y * output_pic->stride_y) + (output_pic->org_x),
                               (uint16_t *)output_pic->buffer_cb +
                                   (((output_pic->org_y) >> ss_y) * output_pic->stride_cb) +
...
static EbErrorType copy_recon_enc(SequenceControlSet *scs, EbPictureBufferDesc *recon_picture_src,
    ...
    recon_picture_dst->buffer_enable_mask = scs->seq_header.color_config.mono_chrome
