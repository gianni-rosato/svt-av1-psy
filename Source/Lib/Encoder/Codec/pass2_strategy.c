...
static int av1_calc_pframe_target_size_one_pass_cbr(
    PictureParentControlSet *pcs_ptr, FRAME_UPDATE_TYPE frame_update_type) {
  SequenceControlSet *scs_ptr = pcs_ptr->scs_ptr;
  EncodeContext *encode_context_ptr = scs_ptr->encode_context_ptr;
  RATE_CONTROL *const rc = &encode_context_ptr->rc;
  const RateControlCfg *const rc_cfg = &encode_context_ptr->rc_cfg;
...
void set_rc_param(SequenceControlSet *scs_ptr) {
    ...
    encode_context_ptr->rc_cfg.mode = scs_ptr->static_config.rate_control_mode == 1 ? AOM_VBR :
                                     (scs_ptr->static_config.rate_control_mode == 2 ? AOM_CBR
                                                                                    : AOM_Q);
#else
...
#if FTR_2PASS_CBR || FTR_1PASS_CBR
    encode_context_ptr->rc_cfg.maximum_buffer_size_ms   = is_vbr ? 240000 : scs_ptr->static_config.maximum_buffer_size_ms;
    encode_context_ptr->rc_cfg.starting_buffer_level_ms = is_vbr ? 60000  : scs_ptr->static_config.starting_buffer_level_ms;
    encode_context_ptr->rc_cfg.optimal_buffer_level_ms  = is_vbr ? 60000  : scs_ptr->static_config.optimal_buffer_level_ms;
#else
...
void crf_assign_max_rate(PictureParentControlSet *ppcs_ptr) {
...
#if DEBUG_RC_CAP_LOG
        printf("SW_POC:%lld\t%lld\t%lld\t%d\tboost:%d\n", ppcs_ptr->picture_number, max_bits_sw, available_bit_sw, max_frame_size, rc->kf_boost);
#endif
    }
...
#if DEBUG_RC_CAP_LOG
        printf("SW_POC:%lld\t%lld\t%lld\t%d\tboost:%d\n", ppcs_ptr->picture_number, gf_group_bits, available_bit_sw, max_frame_size, rc->gfu_boost);
#endif
    }
    ...
    if (available_bit_ratio <= OPTIMAL_BUFFER_LEVEL) {
        if (available_bit_ratio > CRITICAL_BUFFER_LEVEL) {
            int max_adjustment = (available_bit_ratio + 20 < available_frames_ratio) ? rc->active_worst_quality : rc->active_worst_quality / 2;
            // Adjust up from assigned QP.
            if (buff_lvl_step && available_bit_ratio < available_frames_ratio + 10) {
                adjustment = (int)(max_adjustment *
                    (OPTIMAL_BUFFER_LEVEL - available_bit_ratio) /
                    buff_lvl_step);
            }
        }
        else {
...
#if DEBUG_RC_CAP_LOG
    printf("SW_POC:%lld\t%lld\t%lld\t%d%%\t%d%%\tadj:\t%d\n", ppcs_ptr->picture_number, max_bits_sw, available_bit_sw, available_bit_ratio, available_frames_ratio, adjustment);
#endif
