...
void aom_comp_avg_upsampled_pred_c(MacroBlockD *xd, const AV1_COMMON *const cm, int mi_row,
                                   int mi_col, const MV *const mv, uint8_t *comp_pred,
                                   const uint8_t *pred, int width, int height, int subpel_x_q3,
                                   int subpel_y_q3, const uint8_t *ref, int ref_stride,
                                   int subpel_search) {
    int i, j;

    aom_upsampled_pred_c(xd,
...
void aom_comp_mask_pred_c(uint8_t *comp_pred, const uint8_t *pred, int width, int height,
                          const uint8_t *ref, int ref_stride, const uint8_t *mask, int mask_stride,
                          int invert_mask) {
    int            i, j;
    ...
    for (i = 0; i < height; ++i) {
        for (j = 0; j < width; ++j) { comp_pred[j] = AOM_BLEND_A64(mask[j], src0[j], src1[j]); }
        comp_pred += width;
        src0 += stride0;
        src1 += stride1;
        mask += mask_stride;
...
void aom_comp_mask_upsampled_pred_c(MacroBlockD *xd, const AV1_COMMON *const cm, int mi_row,
                                    int mi_col, const MV *const mv, uint8_t *comp_pred,
                                    const uint8_t *pred, int width, int height, int subpel_x_q3,
                                    int subpel_y_q3, const uint8_t *ref, int ref_stride,
    ...
    aom_comp_mask_pred_c(
        comp_pred, pred, width, height, ref, ref_stride, mask, mask_stride, invert_mask);
}
