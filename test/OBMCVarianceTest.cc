...
using CalcTargetWeightedPredParam = tuple<CalcTargetWeightedPredFn, CalcTargetWeightedPredFn>;
...
class CalcTargetWeightedPredTest
  ...
  public:
    CalcTargetWeightedPredTest()
        ...
        mask_buf_ref = reinterpret_cast<int32_t *>(malloc(2 * MAX_SB_SQUARE * sizeof(int32_t)));
        mask_buf_tst = reinterpret_cast<int32_t *>(malloc(2 * MAX_SB_SQUARE * sizeof(int32_t)));
        wsrc_buf_ref = reinterpret_cast<int32_t *>(malloc(2 * MAX_SB_SQUARE * sizeof(int32_t)));
        wsrc_buf_tst = reinterpret_cast<int32_t *>(malloc(2 * MAX_SB_SQUARE * sizeof(int32_t)));
        tmp_ref = reinterpret_cast<uint8_t *>(malloc(2 * MAX_SB_SQUARE * sizeof(uint8_t)));
        tmp_tst = reinterpret_cast<uint8_t *>(malloc(2 * MAX_SB_SQUARE * sizeof(uint8_t)));
    ...
    ~CalcTargetWeightedPredTest() {
        if (mask_buf_ref) free(mask_buf_ref);
        if (mask_buf_tst) free(mask_buf_tst);
        if (wsrc_buf_ref) free(wsrc_buf_ref);
        if (wsrc_buf_tst) free(wsrc_buf_tst);
        if (tmp_ref) free(tmp_ref);
        if (tmp_tst) free(tmp_tst);
    }
  ...
  protected:
    void run_test() {
        ...
        for (uint32_t overlap_cnt = 0; overlap_cnt < sizeof(overlap_tab) / sizeof(overlap_tab[0]); overlap_cnt++) {
            int width = overlap_tab[overlap_cnt];
            xd.n4_w = 5 + (width >> MI_SIZE_LOG2);
            calc_target_weighted_pred_ctxt ctxt_ref = {mask_buf_ref, wsrc_buf_ref, tmp_ref,stride, width};
            calc_target_weighted_pred_ctxt ctxt_tst = {mask_buf_tst, wsrc_buf_tst, tmp_tst,stride, width};
            for (uint32_t i = 0; i < test_num; i++) {
                ...
                if (memcmp(mask_buf_ref, mask_buf_tst, sizeof(int32_t) * 2 * MAX_SB_SQUARE) != 0) {
                    for (uint32_t j = 0; j < 2 * MAX_SB_SQUARE; j++)
                        if (mask_buf_ref[j] != mask_buf_tst[j]) {
                            printf("error mask at idx=%d\texpected=%d\tcalculated=%d\n", j, mask_buf_ref[j], mask_buf_tst[j]);
                            ASSERT(0);
                        }
                }
                if (memcmp(wsrc_buf_ref, wsrc_buf_tst, sizeof(int32_t) * 2 * MAX_SB_SQUARE) != 0) {
                    for (uint32_t j = 0; j < 2 * MAX_SB_SQUARE; j++)
                        if (wsrc_buf_ref[j] != wsrc_buf_tst[j]) {
                            printf("error wsrc at idx=%d\texpected=%d\tcalculated=%d\n", j, wsrc_buf_ref[j], wsrc_buf_tst[j]);
                            ASSERT(0);
                        }
                }
                if (memcmp(tmp_ref, tmp_tst, sizeof(uint8_t) * 2 * MAX_SB_SQUARE) != 0) {
                    for (uint32_t j = 0; j < 2 * MAX_SB_SQUARE; j++)
                        if (tmp_ref[j] != tmp_tst[j]) {
                            printf("error tmp at idx=%d\texpected=%d\tcalculated=%d\n", j, tmp_ref[j], tmp_tst[j]);
                            ASSERT(0);
                        }
                }
            }
